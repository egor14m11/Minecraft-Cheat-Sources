...
public class BiomePlains extends Biome
{
    ...
    protected BiomePlains(boolean p_i46699_1_, Biome.BiomeProperties properties)
    {
        ...
        theBiomeDecorator.extraTreeChance = 0.05F;
    ...
    {
        double d0 = Biome.GRASS_COLOR_NOISE.getValue((double)pos.getX() / 200.0D, (double)pos.getZ() / 200.0D);
    ...
    {
        double d0 = Biome.GRASS_COLOR_NOISE.getValue((double)(pos.getX() + 8) / 200.0D, (double)(pos.getZ() + 8) / 200.0D);
        ...
        else
        {
            ...
            for (int i = 0; i < 7; ++i)
            {
                ...
                Biome.DOUBLE_PLANT_GENERATOR.generate(worldIn, rand, pos.add(j, l, k));
            }
        }
    ...
    {
        return rand.nextInt(3) == 0 ? Biome.BIG_TREE_FEATURE : Biome.TREE_FEATURE;
    }
}
