...
public class BiomeForest extends Biome
{
    ...
    public BlockFlower.EnumFlowerType pickRandomFlower(Random rand, BlockPos pos)
    {
        if (type == BiomeForest.Type.FLOWER)
        {
            double d0 = MathHelper.clamp((1.0D + Biome.GRASS_COLOR_NOISE.getValue((double)pos.getX() / 48.0D, (double)pos.getZ() / 48.0D)) / 2.0D, 0.0D, 0.9999D);
    ...
    protected void addMushrooms(World p_185379_1_, Random p_185379_2_, BlockPos p_185379_3_)
    {
        for (int i = 0; i < 4; ++i)
        {
            for (int j = 0; j < 4; ++j)
            {
                ...
                {
                    WorldGenAbstractTree worldgenabstracttree = genBigTreeChance(p_185379_2_);
    ...
    protected void addDoublePlants(World p_185378_1_, Random p_185378_2_, BlockPos p_185378_3_, int p_185378_4_)
    {
        for (int i = 0; i < p_185378_4_; ++i)
        {
            ...
            for (int k = 0; k < 5; ++k)
            {
                ...
                if (Biome.DOUBLE_PLANT_GENERATOR.generate(p_185378_1_, p_185378_2_, new BlockPos(p_185378_3_.getX() + l, j1, p_185378_3_.getZ() + i1)))
                {
                    break;
                }
            }
