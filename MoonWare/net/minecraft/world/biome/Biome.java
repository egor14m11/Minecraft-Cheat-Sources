...
public abstract class Biome
{
    ...
    public static final RegistryNamespaced<Namespaced, Biome> REGISTRY = new RegistryNamespaced<Namespaced, Biome>();
    ...
    public final float getFloatTemperature(BlockPos pos)
    {
        if (pos.getY() > 64)
        {
            ...
            return getTemperature() - (f + (float)pos.getY() - 64.0F) * 0.05F / 30.0F;
        }
        else
        {
    ...
    {
        if ((double) getTemperature() < 0.2D)
        {
            return Biome.TempCategory.COLD;
        }
        else
        {
            return (double) getTemperature() < 1.0D ? Biome.TempCategory.MEDIUM : Biome.TempCategory.WARM;
        }
    }
    ...
    {
        REGISTRY.register(id, new Namespaced(name), biome);

        if (biome.isMutation())
        {
            MUTATION_TO_BASE_ID_MAP.put(biome, getIdForBiome(REGISTRY.getObject(new Namespaced(biome.baseBiomeRegName))));
        }
    }
