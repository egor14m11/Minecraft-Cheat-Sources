...
public class Teleporter
{
    ...
    public boolean placeInExistingPortal(Entity entityIn, float rotationYaw)
    {
            ...
            for (int i1 = -128; i1 <= 128; ++i1)
            {
                ...
                for (int j1 = -128; j1 <= 128; ++j1)
                {
                    for (BlockPos blockpos1 = blockpos3.add(i1, worldServerInstance.getActualHeight() - 1 - blockpos3.getY(), j1); blockpos1.getY() >= 0; blockpos1 = blockpos2)
                    {
                        ...
                        {
                            for (blockpos2 = blockpos1.down(); worldServerInstance.getBlockState(blockpos2).getBlock() == Blocks.PORTAL; blockpos2 = blockpos2.down())
                            {
                                blockpos1 = blockpos2;
                            }
        ...
        if (d0 >= 0.0D)
        {
            if (flag)
            {
                destinationCoordinateCache.put(l, new Teleporter.PortalPosition(blockpos, worldServerInstance.getTotalWorldTime()));
            }

            double d5 = (double)blockpos.getX() + 0.5D;
            double d7 = (double)blockpos.getZ() + 0.5D;
            BlockPattern.PatternHelper blockpattern$patternhelper = Blocks.PORTAL.createPatternHelper(worldServerInstance, blockpos);
