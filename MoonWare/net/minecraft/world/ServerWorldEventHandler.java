...
public class ServerWorldEventHandler implements IWorldEventListener
{
    ...
    public void onEntityAdded(Entity entityIn)
    {
        ...
        {
            theWorldServer.provider.onPlayerAdded((EntityPlayerMP)entityIn);
        }
    }
    ...
    public void onEntityRemoved(Entity entityIn)
    {
        ...
        theWorldServer.getScoreboard().removeEntity(entityIn);

        if (entityIn instanceof EntityPlayerMP)
        {
            theWorldServer.provider.onPlayerRemoved((EntityPlayerMP)entityIn);
        }
    }

    public void playSoundToAllNearExcept(@Nullable EntityPlayer player, SoundEvent soundIn, SoundCategory category, double x, double y, double z, float volume, float pitch)
    {
        mcServer.getPlayerList().sendToAllNearExcept(player, x, y, z, volume > 1.0F ? (double)(16.0F * volume) : 16.0D, theWorldServer.provider.getDimensionType().getId(), new SPacketSoundEffect(soundIn, category, x, y, z, volume, pitch));
    }
    ...
    {
        theWorldServer.getPlayerChunkMap().markBlockForUpdate(pos);
    }
    ...
    {
        mcServer.getPlayerList().sendToAllNearExcept(player, blockPosIn.getX(), blockPosIn.getY(), blockPosIn.getZ(), 64.0D, theWorldServer.provider.getDimensionType().getId(), new SPacketEffect(type, blockPosIn, data, false));
    }
    ...
    {
        for (EntityPlayerMP entityplayermp : mcServer.getPlayerList().getPlayerList())
        {
            if (entityplayermp != null && entityplayermp.world == theWorldServer && entityplayermp.getEntityId() != breakerId)
            {
