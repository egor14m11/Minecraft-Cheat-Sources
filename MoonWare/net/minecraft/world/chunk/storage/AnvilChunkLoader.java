...
public class AnvilChunkLoader implements IChunkLoader, IThreadedFileIO, IAnvilChunkLoader
{
    ...
    public Chunk loadChunk(World worldIn, int x, int z) throws IOException
    {
        ...
        NBTTagCompound nbttagcompound = chunksToRemove.get(chunkpos);
        ...
        if (nbttagcompound == null)
        {
            ...
            nbttagcompound = field_193416_e.process(FixTypes.CHUNK, CompressedStreamTools.read(datainputstream));
        }
    ...
    public boolean func_191063_a(int p_191063_1_, int p_191063_2_)
    {
        ...
        NBTTagCompound nbttagcompound = chunksToRemove.get(chunkpos);
        return nbttagcompound != null || RegionFileCache.func_191064_f(chunkSaveLocation, p_191063_1_, p_191063_2_);
    }
    ...
    {
        if (!field_193415_c.contains(pos))
        {
            chunksToRemove.put(pos, compound);
        }
    ...
    public boolean writeNextIO()
    {
        ...
        {
            ChunkPos chunkpos = chunksToRemove.keySet().iterator().next();
            boolean lvt_3_1_;

            try
            {
                field_193415_c.add(chunkpos);
                NBTTagCompound nbttagcompound = chunksToRemove.remove(chunkpos);
            ...
            {
                field_193415_c.remove(chunkpos);
            }
    ...
    {
        DataOutputStream dataoutputstream = RegionFileCache.getChunkOutputStream(chunkSaveLocation, pos.chunkXPos, pos.chunkZPos);
    ...
    public void saveExtraData()
    {
        try
        {
            savingExtraData = true;

            while (writeNextIO());
        }
        finally
        {
            savingExtraData = false;
        }
    }
    ...
    private void writeChunkToNBT(Chunk chunkIn, World worldIn, NBTTagCompound compound)
    {
        ...
        if (list != null)
        {
            ...
            {
                NBTTagCompound nbttagcompound1 = new NBTTagCompound();
                Namespaced resourcelocation = Block.REGISTRY.getNameForObject(nextticklistentry.getBlock());
