...
public class ChunkGeneratorEnd implements IChunkGenerator
{
    ...
    {
        worldObj = p_i47241_1_;
        mapFeaturesEnabled = p_i47241_2_;
        field_191061_n = p_i47241_5_;
    ...
    public Chunk provideChunk(int x, int z)
    {
        ...
        {
            endCityGen.generate(worldObj, x, z, chunkprimer);
        }
    ...
    public void populate(int x, int z)
    {
        ...
        if (mapFeaturesEnabled)
        {
            endCityGen.generateStructure(worldObj, rand, new ChunkPos(x, z));
        }

        worldObj.getBiome(blockpos.add(16, 0, 16)).decorate(worldObj, worldObj.rand, blockpos);
        ...
        if (i > 4096L)
        {
            ...
            {
                endIslands.generate(worldObj, rand, blockpos.add(rand.nextInt(16) + 8, 55 + rand.nextInt(16), rand.nextInt(16) + 8));
            ...
            if (getIslandHeightValue(x, z, 1, 1) > 40.0F)
            {
                ...
                for (int k = 0; k < j; ++k)
                {
                    ...
                    int j1 = worldObj.getHeight(blockpos.add(l, 0, i1)).getY();

                    if (j1 > 0)
                    {
                        int k1 = j1 - 1;

                        if (worldObj.isAirBlock(blockpos.add(l, k1 + 1, i1)) && worldObj.getBlockState(blockpos.add(l, k1, i1)).getBlock() == Blocks.END_STONE)
                        {
                            BlockChorusFlower.generatePlant(worldObj, blockpos.add(l, k1 + 1, i1), rand, 8);
                        }
                    }
                }
                ...
                if (rand.nextInt(700) == 0)
                {
                    ...
                    int j2 = worldObj.getHeight(blockpos.add(l1, 0, i2)).getY();

                    if (j2 > 0)
                    {
                        int k2 = j2 + 3 + rand.nextInt(7);
                        BlockPos blockpos1 = blockpos.add(l1, k2, i2);
                        (new WorldGenEndGateway()).generate(worldObj, rand, blockpos1);
                        TileEntity tileentity = worldObj.getTileEntity(blockpos1);

                        if (tileentity instanceof TileEntityEndGateway)
                        {
                            TileEntityEndGateway tileentityendgateway = (TileEntityEndGateway)tileentity;
                            tileentityendgateway.func_190603_b(field_191061_n);
                        }
                    }
                }
            }
    ...
    {
        return "EndCity".equals(structureName) && endCityGen != null ? endCityGen.getClosestStrongholdPos(worldIn, position, p_180513_4_) : null;
    }

    public boolean func_193414_a(World p_193414_1_, String p_193414_2_, BlockPos p_193414_3_)
    {
        return "EndCity".equals(p_193414_2_) && endCityGen != null && endCityGen.isInsideStructure(p_193414_3_);
    }
