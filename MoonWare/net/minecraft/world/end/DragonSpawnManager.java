...
public enum DragonSpawnManager
{
    START {
        public void process(WorldServer worldIn, DragonFightManager manager, List<EntityEnderCrystal> crystals, int ticks, BlockPos pos)
        {
            ...
            manager.setRespawnState(DragonSpawnManager.PREPARING_TO_SUMMON_PILLARS);
        }
    },
    PREPARING_TO_SUMMON_PILLARS {
        public void process(WorldServer worldIn, DragonFightManager manager, List<EntityEnderCrystal> crystals, int ticks, BlockPos pos)
        {
            ...
            {
                manager.setRespawnState(DragonSpawnManager.SUMMONING_PILLARS);
            }
        }
    },
    SUMMONING_PILLARS {
        public void process(WorldServer worldIn, DragonFightManager manager, List<EntityEnderCrystal> crystals, int ticks, BlockPos pos)
        {
            ...
            if (flag || flag1)
            {
                ...
                {
                    manager.setRespawnState(DragonSpawnManager.SUMMONING_DRAGON);
                }
            }
        }
    },
    SUMMONING_DRAGON {
        public void process(WorldServer worldIn, DragonFightManager manager, List<EntityEnderCrystal> crystals, int ticks, BlockPos pos)
        {
            if (ticks >= 100)
            {
                manager.setRespawnState(DragonSpawnManager.END);
