...
public abstract class PlayerList
{
    ...
    public PlayerList(MinecraftServer server)
    {
        ...
        field_192055_p = Maps.newHashMap();
        mcServer = server;
    ...
    public void initializeConnectionToPlayer(NetworkManager netManager, EntityPlayerMP playerIn)
    {
        ...
        {
            textcomponenttranslation = new TranslatableComponent("multiplayer.player.joined", playerIn.getDisplayName());
        }
        else
        {
            textcomponenttranslation = new TranslatableComponent("multiplayer.player.joined.renamed", playerIn.getDisplayName(), s);
        }

        textcomponenttranslation.getStyle().setColor(Formatting.YELLOW);
        sendChatMsg(textcomponenttranslation);
    ...
    protected void writePlayerData(EntityPlayerMP playerIn)
    {
        ...
        PlayerAdvancements playeradvancements = field_192055_p.get(playerIn.getUniqueID());
    ...
    public EntityPlayerMP createPlayerForUser(GameProfile profile)
    {
        ...
        {
            entityplayermp1.connection.func_194028_b(new TranslatableComponent("multiplayer.disconnect.duplicate_login"));
        }

        PlayerInteractionManager playerinteractionmanager = new PlayerInteractionManager(mcServer.worldServerForDimension(0));
        return new EntityPlayerMP(mcServer, mcServer.worldServerForDimension(0), profile, playerinteractionmanager);
    }
    ...
    public EntityPlayerMP recreatePlayerEntity(EntityPlayerMP playerIn, int dimension, boolean conqueredEnd)
    {
        ...
        playerEntityList.remove(playerIn);
        mcServer.worldServerForDimension(playerIn.dimension).removeEntityDangerously(playerIn);
        BlockPos blockpos = playerIn.getBedLocation();
        boolean flag = playerIn.isSpawnForced();
        playerIn.dimension = dimension;
        PlayerInteractionManager playerinteractionmanager = new PlayerInteractionManager(mcServer.worldServerForDimension(playerIn.dimension));
        EntityPlayerMP entityplayermp = new EntityPlayerMP(mcServer, mcServer.worldServerForDimension(playerIn.dimension), playerIn.getGameProfile(), playerinteractionmanager);
    ...
    public void updatePermissionLevel(EntityPlayerMP player)
    {
        ...
        i = mcServer.isSinglePlayer() && mcServer.worldServers[0].getWorldInfo().areCommandsAllowed() ? 4 : i;
    ...
    public void sendMessageToAllTeamMembers(EntityPlayer player, Component message)
    {
    ...
    public void sendMessageToTeamOrAllPlayers(EntityPlayer player, Component message)
    {
        Team team = player.getTeam();
    ...
    public void removeAllPlayers()
    {
        for (int i = 0; i < playerEntityList.size(); ++i)
        {
            (playerEntityList.get(i)).connection.func_194028_b(new TranslatableComponent("multiplayer.disconnect.server_shutdown"));
        }
    }

    public void sendChatMsgImpl(Component component, boolean isSystem)
    {
     ...
     */
    public void sendChatMsg(Component component)
    {
    ...
    public PlayerAdvancements func_192054_h(EntityPlayerMP p_192054_1_)
    {
        ...
        PlayerAdvancements playeradvancements = field_192055_p.get(uuid);

        if (playeradvancements == null)
        {
            File file1 = new File(mcServer.worldServerForDimension(0).getSaveHandler().getWorldDirectory(), "advancements");
            File file2 = new File(file1, uuid + ".json");
            playeradvancements = new PlayerAdvancements(mcServer, file2, p_192054_1_);
            field_192055_p.put(uuid, playeradvancements);
        }
    ...
    {
        for (PlayerAdvancements playeradvancements : field_192055_p.values())
        {
