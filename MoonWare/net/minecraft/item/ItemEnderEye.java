    ...
    public EnumActionResult onItemUse(EntityPlayer stack, World playerIn, BlockPos worldIn, EnumHand pos, EnumFacing hand, float facing, float hitX, float hitY)
    {
        ...
        if (stack.canPlayerEdit(worldIn.offset(hand), hand, itemstack) && iblockstate.getBlock() == Blocks.END_PORTAL_FRAME && !iblockstate.getValue(BlockEndPortalFrame.EYE).booleanValue())
        {
            ...
            else
            {
                ...
                for (int i = 0; i < 16; ++i)
                {
                    double d0 = (float)worldIn.getX() + (5.0F + Item.itemRand.nextFloat() * 6.0F) / 16.0F;
                    double d1 = (float)worldIn.getY() + 0.8125F;
                    double d2 = (float)worldIn.getZ() + (5.0F + Item.itemRand.nextFloat() * 6.0F) / 16.0F;
                    double d3 = 0.0D;
                    double d4 = 0.0D;
                    double d5 = 0.0D;
        ...
        else
        {
            ...
            if (!itemStackIn.isRemote)
            {
                ...
                if (blockpos != null)
                {
                    ...
                    itemStackIn.playSound(null, worldIn.posX, worldIn.posY, worldIn.posZ, SoundEvents.ENTITY_ENDEREYE_LAUNCH, SoundCategory.NEUTRAL, 0.5F, 0.4F / (Item.itemRand.nextFloat() * 0.4F + 0.8F));
