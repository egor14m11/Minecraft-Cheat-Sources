...
public class PotionUtils
{
    ...
    {
        Namespaced resourcelocation = PotionType.REGISTRY.getNameForObject(potionIn);
    ...
    public static void addPotionTooltip(ItemStack itemIn, List<String> lores, float durationFactor)
    {
        ...
        else
        {
            for (PotionEffect potioneffect : list)
            {
                ...
                {
                    lores.add(Formatting.RED + s1);
                }
                else
                {
                    lores.add(Formatting.BLUE + s1);
                }
            }
        }
        ...
        {
            lores.add("");
            lores.add(Formatting.DARK_PURPLE + I18n.translateToLocal("potion.whenDrank"));
            ...
            for (Tuple<String, AttributeModifier> tuple : list1)
            {
                ...
                if (d0 > 0.0D)
                {
                    lores.add(Formatting.BLUE + I18n.translateToLocalFormatted("attribute.modifier.plus." + attributemodifier2.getOperation(), ItemStack.DECIMALFORMAT.format(d1), I18n.translateToLocal("attribute.name." + tuple.getFirst())));
                }
                else if (d0 < 0.0D)
                {
                    d1 = d1 * -1.0D;
                    lores.add(Formatting.RED + I18n.translateToLocalFormatted("attribute.modifier.take." + attributemodifier2.getOperation(), ItemStack.DECIMALFORMAT.format(d1), I18n.translateToLocal("attribute.name." + tuple.getFirst())));
                }
            }
        }
    }
