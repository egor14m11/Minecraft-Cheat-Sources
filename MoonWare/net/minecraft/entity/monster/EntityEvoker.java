...
public class EntityEvoker extends EntitySpellcasterIllager
{
    ...
    public boolean isOnSameTeam(Entity entityIn)
    {
        ...
        {
            return isOnSameTeam(((EntityVex)entityIn).func_190645_o());
        }
    ...
    {
        field_190763_bw = p_190748_1_;
    }

    @Nullable
    private EntitySheep func_190751_dj()
    {
        return field_190763_bw;
    }
    ...
    class AIAttackSpell extends EntitySpellcasterIllager.AIUseSpell
    {
        ...
        protected void func_190868_j()
        {
            ...
            if (getDistanceSqToEntity(entitylivingbase) < 9.0D)
            {
                for (int i = 0; i < 5; ++i)
                {
                    float f1 = f + (float)i * (float)Math.PI * 0.4F;
                    func_190876_a(posX + (double)MathHelper.cos(f1) * 1.5D, posZ + (double)MathHelper.sin(f1) * 1.5D, d0, d1, f1, 0);
                }

                for (int k = 0; k < 8; ++k)
                {
                    float f2 = f + (float)k * (float)Math.PI * 2.0F / 8.0F + ((float)Math.PI * 2F / 5F);
                    func_190876_a(posX + (double)MathHelper.cos(f2) * 2.5D, posZ + (double)MathHelper.sin(f2) * 2.5D, d0, d1, f2, 3);
                }
            }
            else
            {
                for (int l = 0; l < 16; ++l)
                {
                    ...
                    int j = 1 * l;
                    func_190876_a(posX + (double)MathHelper.cos(f) * d2, posZ + (double)MathHelper.sin(f) * d2, d0, d1, f, j);
                }
            }
        }
        ...
        private void func_190876_a(double p_190876_1_, double p_190876_3_, double p_190876_5_, double p_190876_7_, float p_190876_9_, int p_190876_10_)
        {
            ...
            if (flag)
            {
                ...
                world.spawnEntityInWorld(entityevokerfangs);
            }
        }
    ...
    class AICastingSpell extends EntitySpellcasterIllager.AICastingApell
    {
        ...
        public void updateTask()
        {
            ...
            else if (func_190751_dj() != null)
            {
                getLookHelper().setLookPositionWithEntity(func_190751_dj(), (float) getHorizontalFaceSpeed(), (float) getVerticalFaceSpeed());
            }
        }
    }
    ...
    class AISummonSpell extends EntitySpellcasterIllager.AIUseSpell
    {
        ...
        public boolean shouldExecute()
        {
            ...
            {
                int i = world.getEntitiesWithinAABB(EntityVex.class, getEntityBoundingBox().expandXyz(16.0D)).size();
        ...
        protected void func_190868_j()
        {
            for (int i = 0; i < 3; ++i)
            {
                ...
                entityvex.func_190653_a(20 * (30 + rand.nextInt(90)));
                world.spawnEntityInWorld(entityvex);
            }
        }
    ...
    public class AIWololoSpell extends EntitySpellcasterIllager.AIUseSpell
    {
        ...
        public boolean shouldExecute()
        {
            ...
            else if (ticksExisted < field_193322_d)
            {
                return false;
            }
            ...
            {
                List<EntitySheep> list = world.getEntitiesWithinAABB(EntitySheep.class, getEntityBoundingBox().expand(16.0D, 4.0D, 16.0D), field_190879_a);
                ...
                {
                    func_190748_a(list.get(rand.nextInt(list.size())));
                    return true;
                }
            }
        }
        ...
        {
            return func_190751_dj() != null && field_193321_c > 0;
        }
        ...
        protected void func_190868_j()
        {
            EntitySheep entitysheep = func_190751_dj();
