...
{
    private static final Namespaced SIGN_TEXTURE = new Namespaced("textures/entity/sign.png");
    ...
    {
        if (MoonWare.featureManager.getFeatureByClass(NoRender.class).getState() && NoRender.sign.getBoolValue()) {
            return;
        }
        ...
        if (p_192841_9_ < 0)
        {
            for (int j = 0; j < p_192841_1_.signText.length; ++j)
            {
                ...
                {
                    Component itextcomponent = p_192841_1_.signText[j];
                    List<Component> list = GuiUtilRenderComponents.splitText(itextcomponent, 90, fontrenderer, false, true);
                    String s = list != null && !list.isEmpty() ? list.get(0).asFormattedString() : "";
