...
public class ChunkRenderWorker implements Runnable
{
    ...
    public void run()
    {
        while (shouldRun)
        {
            ...
            catch (Throwable throwable)
            {
                ...
                Minecraft.displayCrashReport(Minecraft.populateReport(crashreport));
                return;
            }
        }
    }
    ...
    protected void processTask(ChunkCompileTaskGenerator generator) throws InterruptedException
    {
        ...
        Entity entity1 = Minecraft.getRenderViewEntity();
        ...
        else
        {
            ...
            CompiledChunk compiledchunk1 = generator.getCompiledChunk();
            ...
            if (chunkcompiletaskgenerator$type == ChunkCompileTaskGenerator.Type.REBUILD_CHUNK)
            {
                for (BlockRenderLayer blockrenderlayer : BlockRenderLayer.values())
                {
                    ...
                    {
                        arraylist1.add(chunkRenderDispatcher.uploadChunk(blockrenderlayer, generator.getRegionRenderCacheBuilder().getWorldRendererByLayer(blockrenderlayer), generator.getRenderChunk(), compiledchunk1, generator.getDistanceSq()));
                    }
                }
            }
            else if (chunkcompiletaskgenerator$type == ChunkCompileTaskGenerator.Type.RESORT_TRANSPARENCY)
            {
                arraylist1.add(chunkRenderDispatcher.uploadChunk(BlockRenderLayer.TRANSLUCENT, generator.getRegionRenderCacheBuilder().getWorldRendererByLayer(BlockRenderLayer.TRANSLUCENT), generator.getRenderChunk(), compiledchunk1, generator.getDistanceSq()));
            }

            ListenableFuture<List<Object>> listenablefuture = Futures.allAsList(arraylist1);
            ...
            Futures.addCallback(listenablefuture, new FutureCallback<List<Object>>()
            {
                public void onSuccess(@Nullable List<Object> p_onSuccess_1_)
                {
                    ...
                    try
                    {
                        if (generator.getStatus() != ChunkCompileTaskGenerator.Status.UPLOADING)
                        {
                            if (!generator.isFinished())
                            {
                                LOGGER.warn("Chunk render task was {} when I expected it to be uploading; aborting task", generator.getStatus());
                            }
                ...
                public void onFailure(Throwable p_onFailure_1_)
                {
                    ...
                    {
                        Minecraft.displayCrashReport(CrashReport.makeCrashReport(p_onFailure_1_, "Rendering chunk"));
                    }
                }
