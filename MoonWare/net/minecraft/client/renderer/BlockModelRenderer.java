...
public class BlockModelRenderer
{
    ...
    private static final BlockRenderLayer[] OVERLAY_LAYERS = {BlockRenderLayer.CUTOUT, BlockRenderLayer.CUTOUT_MIPPED, BlockRenderLayer.TRANSLUCENT};
    ...
    public boolean renderModel(IBlockAccess worldIn, IBakedModel modelIn, IBlockState stateIn, BlockPos posIn, BufferBuilder buffer, boolean checkSides, long rand)
    {
        ...
        try
        {
            ...
            boolean flag1 = flag ? renderModelSmooth(worldIn, modelIn, stateIn, posIn, buffer, checkSides, rand) : renderModelFlat(worldIn, modelIn, stateIn, posIn, buffer, checkSides, rand);

            if (flag1)
            {
                renderOverlayModels(worldIn, modelIn, stateIn, posIn, buffer, checkSides, rand, renderenv, flag);
            }
    ...
    private void renderQuadsSmooth(IBlockAccess p_renderQuadsSmooth_1_, IBlockState p_renderQuadsSmooth_2_, BlockPos p_renderQuadsSmooth_3_, BufferBuilder p_renderQuadsSmooth_4_, List<BakedQuad> p_renderQuadsSmooth_5_, RenderEnv p_renderQuadsSmooth_6_)
    {
        ...
        for (int j = p_renderQuadsSmooth_5_.size(); i < j; ++i)
        {
            ...
            fillQuadBounds(p_renderQuadsSmooth_2_, bakedquad.getVertexData(), bakedquad.getFace(), afloat, bitset);
            ...
            else
            {
                ...
                if (l == -1)
                {
                    k = blockColors.colorMultiplier(p_renderQuadsSmooth_2_, p_renderQuadsSmooth_1_, p_renderQuadsSmooth_3_, bakedquad.getTintIndex());
                }
    ...
    private void renderQuadsFlat(IBlockAccess p_renderQuadsFlat_1_, IBlockState p_renderQuadsFlat_2_, BlockPos p_renderQuadsFlat_3_, int p_renderQuadsFlat_4_, boolean p_renderQuadsFlat_5_, BufferBuilder p_renderQuadsFlat_6_, List<BakedQuad> p_renderQuadsFlat_7_, RenderEnv p_renderQuadsFlat_8_)
    {
        ...
        for (int j = p_renderQuadsFlat_7_.size(); i < j; ++i)
        {
            ...
            {
                fillQuadBounds(p_renderQuadsFlat_2_, bakedquad.getVertexData(), bakedquad.getFace(), null, bitset);
            ...
            else
            {
                ...
                if (l == -1)
                {
                    k = blockColors.colorMultiplier(p_renderQuadsFlat_2_, p_renderQuadsFlat_1_, p_renderQuadsFlat_3_, bakedquad.getTintIndex());
                }
    ...
    private void renderOverlayModels(IBlockAccess p_renderOverlayModels_1_, IBakedModel p_renderOverlayModels_2_, IBlockState p_renderOverlayModels_3_, BlockPos p_renderOverlayModels_4_, BufferBuilder p_renderOverlayModels_5_, boolean p_renderOverlayModels_6_, long p_renderOverlayModels_7_, RenderEnv p_renderOverlayModels_9_, boolean p_renderOverlayModels_10_)
    {
                ...
                if (listquadsoverlay.size() > 0)
                {
                    ...
                    if (regionrendercachebuilder != null)
                    {
                        ...
                        for (int j = 0; j < listquadsoverlay.size(); ++j)
                        {
                            ...
                            {
                                renderQuadsSmooth(p_renderOverlayModels_1_, iblockstate, p_renderOverlayModels_4_, bufferbuilder, list, p_renderOverlayModels_9_);
                            }
                            else
                            {
                                int k = iblockstate.getPackedLightmapCoords(p_renderOverlayModels_1_, p_renderOverlayModels_4_.offset(bakedquad.getFace()));
                                renderQuadsFlat(p_renderOverlayModels_1_, iblockstate, p_renderOverlayModels_4_, k, false, bufferbuilder, list, p_renderOverlayModels_9_);
                            }
                        }
                    }
        ...
        if (Config.isBetterSnow() && !p_renderOverlayModels_9_.isBreakingAnimation() && BetterSnow.shouldRender(p_renderOverlayModels_1_, p_renderOverlayModels_3_, p_renderOverlayModels_4_))
        {
            ...
            renderModel(p_renderOverlayModels_1_, ibakedmodel, iblockstate1, p_renderOverlayModels_4_, p_renderOverlayModels_5_, p_renderOverlayModels_6_, p_renderOverlayModels_7_);
        }
    }
    ...
    public static class AmbientOcclusionFace
    {
        ...
        public void updateVertexBrightness(IBlockAccess worldIn, IBlockState state, BlockPos centerPos, EnumFacing direction, float[] faceShape, BitSet shapeState)
        {
            ...
            f = fixAoLightValue(f);
            f1 = fixAoLightValue(f1);
            f2 = fixAoLightValue(f2);
            f3 = fixAoLightValue(f3);
            ...
            if (shapeState.get(1) && blockmodelrenderer$enumneighborinfo.doNonCubicWeight)
            {
                ...
                vertexColorMultiplier[blockmodelrenderer$vertextranslations.vert0] = f29 * f9 + f30 * f10 + f31 * f11 + f32 * f12;
                vertexColorMultiplier[blockmodelrenderer$vertextranslations.vert1] = f29 * f13 + f30 * f14 + f31 * f15 + f32 * f16;
                vertexColorMultiplier[blockmodelrenderer$vertextranslations.vert2] = f29 * f17 + f30 * f18 + f31 * f19 + f32 * f20;
                vertexColorMultiplier[blockmodelrenderer$vertextranslations.vert3] = f29 * f21 + f30 * f22 + f31 * f23 + f32 * f24;
                ...
                vertexBrightness[blockmodelrenderer$vertextranslations.vert0] = getVertexBrightness(i2, j2, k2, l2, f9, f10, f11, f12);
                vertexBrightness[blockmodelrenderer$vertextranslations.vert1] = getVertexBrightness(i2, j2, k2, l2, f13, f14, f15, f16);
                vertexBrightness[blockmodelrenderer$vertextranslations.vert2] = getVertexBrightness(i2, j2, k2, l2, f17, f18, f19, f20);
                vertexBrightness[blockmodelrenderer$vertextranslations.vert3] = getVertexBrightness(i2, j2, k2, l2, f21, f22, f23, f24);
            }
            else
            {
                ...
                vertexColorMultiplier[blockmodelrenderer$vertextranslations.vert0] = f5;
                vertexColorMultiplier[blockmodelrenderer$vertextranslations.vert1] = f6;
                vertexColorMultiplier[blockmodelrenderer$vertextranslations.vert2] = f7;
                vertexColorMultiplier[blockmodelrenderer$vertextranslations.vert3] = f8;
            }
        }
