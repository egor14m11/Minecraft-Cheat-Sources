...
public class OpenGlHelper
{
    ...
    public static void initializeTextures()
    {
        ...
        try (Scanner sc = new Scanner(Runtime.getRuntime().exec(new String[] {"cmd.exe", "/c", "wmic cpu get name"}).getInputStream())) {
            while (sc.hasNextLine()) {
                String sn = sc.nextLine();
                if (sn.trim().equalsIgnoreCase("Name") || sn.trim().isEmpty()) continue;
                cpu = sn.trim();
                break;
            }
        }
    ...
    public static boolean useVbo()
    {
        ...
        {
            return vboSupported && Minecraft.gameSettings.useVbo;
        }
    }
