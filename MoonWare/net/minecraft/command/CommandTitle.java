...
public class CommandTitle extends CommandBase
{
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException
    {
        ...
        else
        {
            ...
            EntityPlayerMP entityplayermp = CommandBase.getPlayer(server, sender, args[0]);
            ...
            if (spackettitle$type != SPacketTitle.Type.CLEAR && spackettitle$type != SPacketTitle.Type.RESET)
            {
                if (spackettitle$type == SPacketTitle.Type.TIMES)
                {
                    ...
                    {
                        int i = CommandBase.parseInt(args[2]);
                        int j = CommandBase.parseInt(args[3]);
                        int k = CommandBase.parseInt(args[4]);
                        SPacketTitle spackettitle2 = new SPacketTitle(i, j, k);
                        entityplayermp.connection.sendPacket(spackettitle2);
                        CommandBase.notifyCommandListener(sender, this, "commands.title.success");
                    }
                }
                ...
                {
                    String s = CommandBase.buildString(args, 2);
                    ...
                    SPacketTitle spackettitle1 = new SPacketTitle(spackettitle$type, Components.processComponent(sender, itextcomponent, entityplayermp));
