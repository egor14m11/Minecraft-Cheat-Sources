...
public class CommandReplaceItem extends CommandBase
{
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException
    {
        ...
        else
        {
            ...
            ++i;
            int k = args.length > i ? CommandBase.parseInt(args[i++], 1, item.getItemStackLimit()) : 1;
            int l = args.length > i ? CommandBase.parseInt(args[i++]) : 0;
            ItemStack itemstack = new ItemStack(item, k, l);

            if (args.length > i)
            {
                String s1 = CommandBase.buildString(args, i);
    ...
    public List<String> getTabCompletionOptions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos pos)
    {
        ...
        {
            return CommandBase.getListOfStringsMatchingLastWord(args, SHORTCUTS.keySet());
        }
    }
