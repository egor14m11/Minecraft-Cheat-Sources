...
public class CommandClearInventory extends CommandBase
{
    ...
    {
        EntityPlayerMP entityplayermp = args.length == 0 ? CommandBase.getCommandSenderAsPlayer(sender) : CommandBase.getPlayer(server, sender, args[0]);
        ...
        else
        {
            ...
            else
            {
                if (j == 0)
                {
                    sender.addChatMessage(new TranslatableComponent("commands.clear.testing", entityplayermp.getName(), k));
                }
                else
                {
