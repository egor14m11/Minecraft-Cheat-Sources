...
public class CommandSpreadPlayers extends CommandBase
{
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException
    {
        ...
        else
        {
            ...
            double d0 = CommandBase.parseDouble(blockpos.getX(), args[i++], true);
            double d1 = CommandBase.parseDouble(blockpos.getZ(), args[i++], true);
            double d2 = CommandBase.parseDouble(args[i++], 0.0D);
            double d3 = CommandBase.parseDouble(args[i++], d2 + 1.0D);
            boolean flag = CommandBase.parseBoolean(args[i++]);
            ...
            {
                sender.addChatMessage(new TranslatableComponent("commands.spreadplayers.spreading." + (flag ? "teams" : "players"), list.size(), d3, d0, d1, d2));
    ...
    private void spread(ICommandSender sender, List<Entity> p_110669_2_, CommandSpreadPlayers.Position pos, double spreadDistance, double maxRange, World worldIn, boolean respectTeams) throws CommandException
    {
        ...
        {
            sender.addChatMessage(new TranslatableComponent("commands.spreadplayers.info." + (respectTeams ? "teams" : "players"), String.format("%.2f", d4), i));
        }
    }
