...
public class PathNavigateGround extends PathNavigate
{
    ...
    {
        return theEntity.onGround || getCanSwim() && isInLiquid() || theEntity.isRiding();
    }

    protected Vec3d getEntityPosition()
    {
        return new Vec3d(theEntity.posX, getPathablePosY(), theEntity.posZ);
    }
    ...
    public Path getPathToPos(BlockPos pos)
    {
        if (worldObj.getBlockState(pos).getMaterial() == Material.AIR)
        {
            ...
            for (blockpos = pos.down(); blockpos.getY() > 0 && worldObj.getBlockState(blockpos).getMaterial() == Material.AIR; blockpos = blockpos.down())
            {
            }
            ...
            while (blockpos.getY() < worldObj.getHeight() && worldObj.getBlockState(blockpos).getMaterial() == Material.AIR)
            {
        ...
        if (!worldObj.getBlockState(pos).getMaterial().isSolid())
        {
        ...
        else
        {
            ...
            for (blockpos1 = pos.up(); blockpos1.getY() < worldObj.getHeight() && worldObj.getBlockState(blockpos1).getMaterial().isSolid(); blockpos1 = blockpos1.up())
            {
            }
        ...
        else
        {
            for (int k = i; k < i + sizeX; ++k)
            {
                for (int l = j; l < j + sizeZ; ++l)
                {
                    ...
                    {
                        PathNodeType pathnodetype = nodeProcessor.getPathNodeType(worldObj, k, y - 1, l, theEntity, sizeX, sizeY, sizeZ, true, true);
                        ...
                        pathnodetype = nodeProcessor.getPathNodeType(worldObj, k, y, l, theEntity, sizeX, sizeY, sizeZ, true, true);
                        float f = theEntity.getPathPriority(pathnodetype);
    ...
    {
        nodeProcessor.setCanBreakDoors(canBreakDoors);
    }
