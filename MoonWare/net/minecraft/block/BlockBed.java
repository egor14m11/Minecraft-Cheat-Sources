...
public class BlockBed extends BlockHorizontal implements ITileEntityProvider
{
        ...
        else
        {
            ...
            if (worldIn.provider.canRespawnHere() && worldIn.getBiome(pos) != Biomes.HELL)
            {
                if (state.getValue(OCCUPIED).booleanValue())
                {
                    ...
                    {
                        playerIn.addChatComponentMessage(new TranslatableComponent("tile.bed.occupied"), true);
                        return true;
                    }
                ...
                else
                {
                    ...
                    {
                        playerIn.addChatComponentMessage(new TranslatableComponent("tile.bed.noSleep"), true);
                    }
                    else if (entityplayer$sleepresult == EntityPlayer.SleepResult.NOT_SAFE)
                    {
                        playerIn.addChatComponentMessage(new TranslatableComponent("tile.bed.notSafe"), true);
                    }
                    else if (entityplayer$sleepresult == EntityPlayer.SleepResult.TOO_FAR_AWAY)
                    {
                        playerIn.addChatComponentMessage(new TranslatableComponent("tile.bed.tooFarAway"), true);
                    }
    ...
    public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state)
    {
        ...
        if (state.getValue(PART) == BlockBed.EnumPartType.FOOT)
        {
            blockpos = pos.offset(state.getValue(BlockHorizontal.FACING));
        }
    ...
    public IBlockState getStateFromMeta(int meta)
    {
        ...
        return (meta & 8) > 0 ? getDefaultState().withProperty(PART, BlockBed.EnumPartType.HEAD).withProperty(BlockHorizontal.FACING, enumfacing).withProperty(OCCUPIED, Boolean.valueOf((meta & 4) > 0)) : getDefaultState().withProperty(PART, BlockBed.EnumPartType.FOOT).withProperty(BlockHorizontal.FACING, enumfacing);
    }
    ...
    {
        return new BlockStateContainer(this, BlockHorizontal.FACING, PART, OCCUPIED);
    }
