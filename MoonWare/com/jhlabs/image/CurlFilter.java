...
public class CurlFilter extends TransformFilter {
	 ...
	 */
	public CurlFilter() {
		setEdgeAction(TransformFilter.ZERO);
	}
...
*/
	...
	static class Sampler {
		...
		private final int getPixel(int[] pixels, int x, int y, int width, int height ) {
			if (x < 0 || x >= width || y < 0 || y >= height) {
				switch (edgeAction) {
				case TransformFilter.ZERO:
				default:
					return 0;
				case TransformFilter.WRAP:
					return pixels[(ImageMath.mod(y, height) * width) + ImageMath.mod(x, width)];
				case TransformFilter.CLAMP:
    ...
    public BufferedImage filter( BufferedImage src, BufferedImage dst ) {
		...
		if ( interpolation == TransformFilter.NEAREST_NEIGHBOUR)
