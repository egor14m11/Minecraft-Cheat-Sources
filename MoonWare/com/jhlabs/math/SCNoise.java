	...
	public float evaluate(float x, float y) {
		...
		for (i = -m; i <= m; i++) {
		  for (j = -m; j <= m; j++) {
			...
			h = perm[(ix+i + perm[(iy+j)& SCNoise.TABMASK])& SCNoise.TABMASK];
			
			for (n = SCNoise.NIMPULSES; n > 0; n--, h = (h+1) & SCNoise.TABMASK) {
				/* Convolve filter and impulse. */
				int h4 = h*4;
				dx = fx - (i + SCNoise.impulseTab[h4++]);
				dy = fy - (j + SCNoise.impulseTab[h4++]);
				distsq = dx*dx + dy*dy;
				sum += catrom2(distsq) * SCNoise.impulseTab[h4];
			}
		  }
		}

		return sum / SCNoise.NIMPULSES;
	}
	...
	public float evaluate(float x, float y, float z) {
		...
		for (i = -m; i <= m; i++) {
		  for (j = -m; j <= m; j++) {
			for (k = -m; k <= m; k++) {
				/* Compute voxel hash code. */
				h = perm[(ix+i + perm[(iy+j + perm[(iz+k)& SCNoise.TABMASK])& SCNoise.TABMASK])& SCNoise.TABMASK];
