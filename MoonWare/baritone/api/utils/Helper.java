...
public interface Helper {
    ...
    static Component getPrefix() {
        ...
        boolean xd = now.get(Calendar.MONTH) == Calendar.APRIL && now.get(Calendar.DAY_OF_MONTH) <= 3;
        Component baritone = new TextComponent(xd ? "Baritoe" : BaritoneAPI.getSettings().shortBaritonePrefix.value ? "B" : "Baritone");
    ...
    default void logToast(String message) {
        logToast(getPrefix(), new TextComponent(message));
    }
    ...
    default void logDirect(String message, Formatting color, boolean logAsToast) {
        Stream.of(message.split("\n")).forEach(line -> {
            Component component = new TextComponent(line.replace("\t", "    "));
