    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (paginate) {
            ...
            List<? extends Settings.Setting> toPaginate =
                    (viewModified ? modifiedSettings(Baritone.settings()) : Baritone.settings().allSettings).stream()
            ...
            Paginator.paginate(
                    ...
                    setting -> {
                        Component typeComponent = new TextComponent(String.format(
                                " (%s)",
                                settingTypeToString(setting)
                        ));
                        typeComponent.getStyle().setColor(Formatting.DARK_GRAY);
                        Component hoverComponent = new TextComponent("");
                        ...
                        Component component = new TextComponent(setting.getName());
        ...
        if (resetting) {
            if (!args.hasAny()) {
                ...
                modifiedSettings(Baritone.settings()).forEach(Settings.Setting::reset);
        ...
        if (!doingSomething && !args.hasAny()) {
            ...
            if (resetting) {
                ...
                try {
                    parseAndApply(Baritone.settings(), arg, newValue);
                } catch (Throwable t) {
                    t.printStackTrace();
            ...
            Component oldValueComponent = new TextComponent(String.format("Old value: %s", oldValue));
