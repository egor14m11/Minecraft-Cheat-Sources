...
public class ExampleBaritoneControl implements Helper, AbstractGameEventListener {
    ...
    private void logRanCommand(String command, String rest) {
        if (settings.echoCommands.value) {
            ...
            Component component = new TextComponent(String.format("> %s", toDisplay));
            component.getStyle()
    ...
    public boolean runCommand(String msg) {
        if (msg.trim().equalsIgnoreCase("damn")) {
            ...
            try {
                ((IGuiScreen) Minecraft.screen).openLink(new URI("https://www.dominos.com/en/pages/order/"));
        ...
        ArgConsumer argc = new ArgConsumer(manager, pair.getSecond());
        if (!argc.hasAny()) {
        ...
        if (manager.getCommand(pair.getFirst()) != null) {
            logRanCommand(command, rest);
        }
