...
public class BlockStateInterface {
    ...
    public BlockStateInterface(World world, WorldData worldData, boolean copyLoadedChunks) {
        ...
        if (copyLoadedChunks) {
            loadedChunks = new Long2ObjectOpenHashMap<>(worldLoaded); // make a copy that we can safely access from another thread
        } else {
            loadedChunks = worldLoaded; // this will only be used on the main thread
        }
        useTheRealWorld = !Baritone.settings().pathThroughCachedOnly.value;
        if (!Minecraft.getMinecraft().isThisThread()) {
            throw new IllegalStateException();
        }
        isPassableBlockPos = new BlockPos.MutableBlockPos();
        access = new BlockStateInterfaceAccessWrapper(this);
    }
