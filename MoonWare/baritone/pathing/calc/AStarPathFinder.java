...
public final class AStarPathFinder extends AbstractNodeCostSearch {
    ...
    protected Optional<IPath> calculate0(long primaryTimeout, long failureTimeout) {
        ...
        double[] bestHeuristicSoFar = new double[AbstractNodeCostSearch.COEFFICIENTS.length];//keep track of the best node by the metric of (estimatedCostToGoal + cost / COEFFICIENTS[i])
        ...
        double minimumImprovement = Baritone.settings().minimumImprovementRepropagation.value ? AbstractNodeCostSearch.MIN_IMPROVEMENT : 0;
        ...
        while (!openSet.isEmpty() && numEmptyChunk < pathingMaxChunkBorderFetch && !cancelRequested) {
            ...
            for (Moves moves : allMoves) {
                ...
                if (neighbor.cost - tentativeCost > minimumImprovement) {
                    ...
                    for (int i = 0; i < AbstractNodeCostSearch.COEFFICIENTS.length; i++) {
                        double heuristic = neighbor.estimatedCostToGoal + neighbor.cost / AbstractNodeCostSearch.COEFFICIENTS[i];
                        if (bestHeuristicSoFar[i] - heuristic > minimumImprovement) {
                            bestHeuristicSoFar[i] = heuristic;
                            bestSoFar[i] = neighbor;
                            if (failing && getDistFromStartSq(neighbor) > AbstractNodeCostSearch.MIN_DIST_PATH * AbstractNodeCostSearch.MIN_DIST_PATH) {
                                failing = false;
                            }
                        }
                    }
