...
public class MovementDescend extends Movement {
    ...
    public double calculateCost(CalculationContext context) {
        ...
        if (result.y != dest.y) {
            return ActionCosts.COST_INF; // doesn't apply to us, this position is a fall not a descend
        }
        return result.cost;
    }
    ...
    public static void cost(CalculationContext context, int x, int y, int z, int destX, int destZ, MutableMoveResult res) {
        ...
        totalCost += walk + Math.max(ActionCosts.FALL_N_BLOCKS_COST[1], ActionCosts.CENTER_AFTER_FALL_COST);
        res.x = destX;
        res.y = y - 1;
        res.z = destZ;
        res.cost = totalCost;
    ...
    public static boolean dynamicFallCost(CalculationContext context, int x, int y, int z, int destX, int destZ, double frontBreak, IBlockState below, MutableMoveResult res) {
        ...
        for (int fallHeight = 3; true; fallHeight++) {
            ...
            double tentativeCost = ActionCosts.WALK_OFF_BLOCK_COST + ActionCosts.FALL_N_BLOCKS_COST[unprotectedFallHeight] + frontBreak + costSoFar;
            ...
            if (unprotectedFallHeight <= 11 && (ontoBlock.getBlock() == Blocks.VINE || ontoBlock.getBlock() == Blocks.LADDER)) {
                ...
                costSoFar += ActionCosts.FALL_N_BLOCKS_COST[unprotectedFallHeight - 1];// we fall until the top of this block (not including this block)
