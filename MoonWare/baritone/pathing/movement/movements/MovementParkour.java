...
public class MovementParkour extends Movement {
    ...
    private static final BetterBlockPos[] EMPTY = {};
    ...
    public static void cost(CalculationContext context, int x, int y, int z, EnumFacing dir, MutableMoveResult res) {
        ...
        for (int i = verifiedMaxJump; i > 1; i--) {
            ...
            for (int j = 0; j < 5; j++) {
                int againstX = destX + Movement.HORIZONTALS_BUT_ALSO_DOWN_____SO_EVERY_DIRECTION_EXCEPT_UP[j].getFrontOffsetX();
                int againstY = y - 1 + Movement.HORIZONTALS_BUT_ALSO_DOWN_____SO_EVERY_DIRECTION_EXCEPT_UP[j].getFrontOffsetY();
                int againstZ = destZ + Movement.HORIZONTALS_BUT_ALSO_DOWN_____SO_EVERY_DIRECTION_EXCEPT_UP[j].getFrontOffsetZ();
    ...
    private static double costFromJumpDistance(int dist) {
        switch (dist) {
            case 2:
                return ActionCosts.WALK_ONE_BLOCK_COST * 2; // IDK LOL
            case 3:
                return ActionCosts.WALK_ONE_BLOCK_COST * 3;
            case 4:
                return ActionCosts.SPRINT_ONE_BLOCK_COST * 4;
            default:
