    ...
    public static double cost(CalculationContext context, int x, int y, int z) {
        ...
        if (!ladder) {
            if (fromDown.getBlock() == Blocks.LADDER || fromDown.getBlock() == Blocks.VINE) {
                return ActionCosts.COST_INF; // can't pillar from a ladder or vine onto something that isn't also climbable
            }
        ...
        if (MovementHelper.isWater(toBreakBlock) && MovementHelper.isWater(from)) { // TODO should this also be allowed if toBreakBlock is air?
            ...
            if (MovementHelper.isWater(srcUp)) {
                return ActionCosts.LADDER_UP_ONE_COST; // allow ascending pillars of water, but only if we're already in one
            }
        }
        double placeCost = 0;
        if (!ladder) {
