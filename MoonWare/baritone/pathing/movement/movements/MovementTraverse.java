        ...
        if (MovementHelper.canWalkOn(context.bsi, destX, y - 1, destZ, destOn)) {//this is a walk, not a bridge
            ...
            if (MovementHelper.isReplaceable(destX, y - 1, destZ, destOn, context.bsi)) {
                ...
                for (int i = 0; i < 5; i++) {
                    ...
                    int againstY = y - 1 + Movement.HORIZONTALS_BUT_ALSO_DOWN_____SO_EVERY_DIRECTION_EXCEPT_UP[i].getFrontOffsetY();
                ...
                if (srcDown == Blocks.SOUL_SAND || (srcDown instanceof BlockSlab && !((BlockSlab) srcDown).isDouble())) {
                    return ActionCosts.COST_INF; // can't sneak and backplace against soul sand or half slabs (regardless of whether it's top half or bottom half) =/
                }
                if (srcDown == Blocks.FLOWING_WATER || srcDown == Blocks.WATER) {
                    return ActionCosts.COST_INF; // this is obviously impossible
                }
                WC = WC * (ActionCosts.SNEAK_ONE_BLOCK_COST / ActionCosts.WALK_ONE_BLOCK_COST);//since we are sneak backplacing, we are sneaking lol
                return WC + placeCost + hardness1 + hardness2;
            }
            return ActionCosts.COST_INF;
        }
