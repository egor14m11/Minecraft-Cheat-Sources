...
public class MovementAscend extends Movement {
    ...
    public static double cost(CalculationContext context, int x, int y, int z, int destX, int destZ) {
        ...
        if (!MovementHelper.canWalkOn(context.bsi, destX, y, destZ, toPlace)) {
            ...
            for (int i = 0; i < 5; i++) {
                int againstX = destX + Movement.HORIZONTALS_BUT_ALSO_DOWN_____SO_EVERY_DIRECTION_EXCEPT_UP[i].getFrontOffsetX();
                int againstY = y + Movement.HORIZONTALS_BUT_ALSO_DOWN_____SO_EVERY_DIRECTION_EXCEPT_UP[i].getFrontOffsetY();
                int againstZ = destZ + Movement.HORIZONTALS_BUT_ALSO_DOWN_____SO_EVERY_DIRECTION_EXCEPT_UP[i].getFrontOffsetZ();
        ...
        if (jumpingFromBottomSlab && !jumpingToBottomSlab) {
            return ActionCosts.COST_INF;// the only thing we can ascend onto from a bottom slab is another bottom slab
        }
        double walk;
        if (jumpingToBottomSlab) {
            if (jumpingFromBottomSlab) {
                walk = Math.max(ActionCosts.JUMP_ONE_BLOCK_COST, ActionCosts.WALK_ONE_BLOCK_COST); // we hit space immediately on entering this action
                walk += context.jumpPenalty;
            } else {
                walk = ActionCosts.WALK_ONE_BLOCK_COST; // we don't hit space we just walk into the slab
            }
        } else {
