...
public final class BuilderProcess extends BaritoneProcessHelper implements IBuilderProcess {
    ...
    public IBlockState placeAt(int x, int y, int z, IBlockState current) {
        ...
        IBlockState state = schematic.desiredState(x - origin.getX(), y - origin.getY(), z - origin.getZ(), current, approxPlaceable);
            ...
            for (int z = 0; z < schematic.lengthZ(); z++) {
                for (int x = 0; x < schematic.widthX(); x++) {
                    ...
                    if (bcc.bsi.worldContainsLoadedChunk(blockX, blockZ)) { // check if its in render distance, not if its in cache
                        ...
                        if (valid(bcc.bsi.get0(blockX, blockY, blockZ), schematic.desiredState(x, y, z, current, approxPlaceable), false)) {
                    ...
                    if (!observedCompleted.contains(BetterBlockPos.longHash(blockX, blockY, blockZ))
                          && !Baritone.settings().buildSkipBlocks.value.contains(schematic.desiredState(x, y, z, current, approxPlaceable).getBlock())) {
