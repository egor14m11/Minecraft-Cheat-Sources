  ...
  public final void invert()
  {
   ...
   /**
    ...
    public boolean epsilonEquals(Matrix4f m1, float epsilon)
    {
        ...
        boolean status = !(Math.abs(m00 - m1.m00) > epsilon);

        if( Math.abs( m01 - m1.m01) > epsilon) status = false;
        if( Math.abs( m02 - m1.m02) > epsilon) status = false;
        if( Math.abs( m03 - m1.m03) > epsilon) status = false;

        if( Math.abs( m10 - m1.m10) > epsilon) status = false;
        if( Math.abs( m11 - m1.m11) > epsilon) status = false;
        if( Math.abs( m12 - m1.m12) > epsilon) status = false;
        if( Math.abs( m13 - m1.m13) > epsilon) status = false;

        if( Math.abs( m20 - m1.m20) > epsilon) status = false;
        if( Math.abs( m21 - m1.m21) > epsilon) status = false;
        if( Math.abs( m22 - m1.m22) > epsilon) status = false;
        if( Math.abs( m23 - m1.m23) > epsilon) status = false;

        if( Math.abs( m30 - m1.m30) > epsilon) status = false;
        if( Math.abs( m31 - m1.m31) > epsilon) status = false;
        if( Math.abs( m32 - m1.m32) > epsilon) status = false;
        if( Math.abs( m33 - m1.m33) > epsilon) status = false;
