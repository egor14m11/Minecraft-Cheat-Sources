...
import org.moonware.client.feature.impl.player.ClipHelper;
                        ...
                        if (RotationHelper.isLookingAtEntity(mc.player.rotationYaw, mc.player.rotationPitch, 0.15F, 0.15F, 0.15F, entity, ClipHelper.maxDistance.getNumberValue())) {
                            ...
                            if (!playerPosY.equals(entityPosY) && mc.gameSettings.thirdPersonView == 0) {
                                if (RenderHelper.isInViewFrustum(entity)) {
                                    ...
                                    float textWidth = (mc.font.getStringWidth("Target" + " §7" + distanceToY.getY() + "m") * scale);
                                    float tagX = (float) ((posX + diff - textWidth / 2) * scale);
                                    mc.font.drawStringWithShadow("Target" + " §7" + distanceToY.getY() + "m", tagX, (float) endPosY, -1);
                                }
                    ...
                    if (targetHP > 0) {
                        ...
                        if (living && heathPercentage.getBoolValue() && (!(espMode.currentMode.equals("Box")))) {
                            if (heathPercentage.getBoolValue()) {
                                GlStateManager.pushMatrix();
                                mc.font.drawStringWithShadow(string2, (float) posX - 36, (float) ((float) endPosY - hpHeight2), -1);
                                GlStateManager.popMatrix();
                            }
                        }
                        ...
                        if (living && healRect.getBoolValue() && (!(espMode.currentMode.equals("Box")))) {
                            ...
                            switch (healcolorMode.currentMode) {
                                ...
                                case "Rainbow":
                                    ...
                                    break;
                                case "Health":
                                    //colorHeal = PaletteHelper.getColor(((EntityLivingBase) entity).getHealth(), ((EntityLivingBase) entity).getMaxHealth());
                                    break;
                            }
                        ...
                        if (living && tags.getBoolValue() && !MoonWare.featureManager.getFeatureByClass(NameTags.class).getState()) {
                            ...
                            float textWidth = (mc.font.getStringWidth(tag) * scale);
                            float tagX = (float) ((posX + diff - textWidth / 2) * scale);
                            ...
                            mc.font.drawStringWithShadow(tag, tagX, tagY, Color.WHITE.getRGB());
                            GlStateManager.popMatrix();
                        }
                    }
                }
