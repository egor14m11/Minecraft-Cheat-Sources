...
public class ItemESP extends Feature {
    ...
    public void onRender3D(EventRender3D event) {
        for (Entity item : Minecraft.world.loadedEntityList) {
        ...
        for (Entity entity : Minecraft.world.loadedEntityList) {
            if (isValid(entity) && RenderHelper.isInViewFrustum(entity)) {
                ...
                if (position != null) {
                    ...
                    float textWidth = (Minecraft.font.getStringWidth(entityItem.getEntityItem().getDisplayName()) * scale);
                    float tagX = (float) ((posX + diff - textWidth / 2) * scale);
                    if (entityName.getBoolValue()) {
                        Minecraft.font.drawStringWithOutline(entityItem.getEntityItem().getDisplayName(), tagX, (float) posY - 10, -1);
                    }
                }
            }
        }
