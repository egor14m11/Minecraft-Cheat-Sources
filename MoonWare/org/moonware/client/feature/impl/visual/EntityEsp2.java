...
import org.moonware.client.ui.shader.shaders.EntityGlowShader;
...
public class EntityEsp2 extends Feature {
    ...
    public EntityEsp2() {
        super("EntityESP - Glu", "Показывает игроков, ник и их здоровье сквозь стены", Type.Visuals);
    ...
    public void onRender3D(EventRender3D event3D) {
        ...
        if (espMode.currentMode.equals("Glow")) {
            ...
            for (Entity entity : Minecraft.world.loadedEntityList) {
                ...
                Minecraft.getRenderManager().renderEntityStatic(entity, event3D.getPartialTicks(), true);
            }
        ...
        if (espMode.currentMode.equals("Box")) {
            ...
            for (Entity entity : Minecraft.world.loadedEntityList) {
                if (entity instanceof EntityPlayer) {
                    if (!(entity == Minecraft.player && Minecraft.gameSettings.thirdPersonView == 0)) {
        ...
        if (triangleESP.getBoolValue()) {
            ...
            for (EntityPlayer entity : Minecraft.world.playerEntities) {
                ...
                if (entity != null && entity != Minecraft.player) {
                    ...
                    double cos = Math.cos(Minecraft.player.rotationYaw * (Math.PI * 2 / 360));
                    double sin = Math.sin(Minecraft.player.rotationYaw * (Math.PI * 2 / 360));
    ...
    public void onRender2D(EventRender2D event) {
        ...
        for (Entity entity : Minecraft.world.loadedEntityList) {
            ...
            if (isValid(entity) && RenderHelper.isInViewFrustum(entity)) {
                ...
                double height = entity.height + ((entity.isSneaking() || (entity == Minecraft.player && Minecraft.player.isSneaking()) ? -0.3D : 0.2D));
                ...
                /*
                    ...
                    /*CLIP HELPER*/
/*
                    if (MoonWare.featureManager.getFeatureByClass(ClipHelper.class).getState()) {
                    ...
                    if (targetHP > 0) {
                        ...
                        if (living && tags.getBoolValue() && !MoonWare.featureManager.getFeatureByClass(NameTags.class).getState()) {
    ...
    private boolean isValid(Entity entity) {
        if (Minecraft.gameSettings.thirdPersonView == 0 && entity == Minecraft.player)
            return false;
        if (entity.isDead)
            return false;
