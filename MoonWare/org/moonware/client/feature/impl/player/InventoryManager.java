...
public class InventoryManager extends Feature {
    ...
    public InventoryManager() {
        super("InventoryManager", "Чистит, сортирует инвентарь за вас", Type.Other);
    ...
    public void onPreMotion(EventPreMotion eventPre) {
        ...
        if (!(Minecraft.screen instanceof GuiInventory) && (openinv.getBoolValue()))
            return;
        if (MovementHelper.isMoving() && (nomoveswap.getBoolValue())) {
            return;
        }
        if (Minecraft.screen == null || Minecraft.screen instanceof GuiInventory || Minecraft.screen instanceof ChatScreen) {
            if (timer.hasReached(delay) && weaponSlot >= 36) {
                if (!Minecraft.player.inventoryContainer.getSlot(weaponSlot).getHasStack()) {
                    ...
                    if (!isBestWeapon(Minecraft.player.inventoryContainer.getSlot(weaponSlot).getStack())) {
                        getBestWeapon(weaponSlot);
                    }
                }
            }
    ...
    public void swap(int slot, int hotbarSlot) {
        Minecraft.playerController.windowClick(Minecraft.player.inventoryContainer.windowId, slot, hotbarSlot, ClickType.SWAP, Minecraft.player);
    }

    public void drop(int slot) {
        Minecraft.playerController.windowClick(Minecraft.player.inventoryContainer.windowId, slot, 1, ClickType.THROW, Minecraft.player);
    }
    ...
    public boolean shouldDrop(ItemStack stack, int slot) {
        ...
        if ((slot == weaponSlot && isBestWeapon(Minecraft.player.inventoryContainer.getSlot(weaponSlot).getStack())) ||
                (slot == pickaxeSlot && isBestPickaxe(Minecraft.player.inventoryContainer.getSlot(pickaxeSlot).getStack()) && pickaxeSlot >= 0) ||
                (slot == axeSlot && isBestAxe(Minecraft.player.inventoryContainer.getSlot(axeSlot).getStack()) && axeSlot >= 0) ||
                (slot == shovelSlot && isBestShovel(Minecraft.player.inventoryContainer.getSlot(shovelSlot).getStack()) && shovelSlot >= 0)) {
            return false;
        }
                    ...
                    if (!isBestWeapon(is))
                        if (!Minecraft.player.inventoryContainer.getSlot(pickaxeSlot).getHasStack()) {
                            ...
                            if (delay1.getNumberValue() > 0)
                                return;
                        } else if (!isBestPickaxe(Minecraft.player.inventoryContainer.getSlot(pickaxeSlot).getStack())) {
                            swap(i, pickaxeSlot - 36);
                            timer.reset();
                            if (delay1.getNumberValue() > 0)
                                return;
                ...
                if (isBestShovel(is) && shovelSlot != i) {
                    if (!isBestWeapon(is))
                        if (!Minecraft.player.inventoryContainer.getSlot(shovelSlot).getHasStack()) {
                            ...
                            if (delay1.getNumberValue() > 0)
                                return;
                        } else if (!isBestShovel(Minecraft.player.inventoryContainer.getSlot(shovelSlot).getStack())) {
                            swap(i, shovelSlot - 36);
                            timer.reset();
                            if (delay1.getNumberValue() > 0)
                                return;
                ...
                if (isBestAxe(is) && axeSlot != i) {
                    if (!isBestWeapon(is))
                        if (!Minecraft.player.inventoryContainer.getSlot(axeSlot).getHasStack()) {
                            ...
                            if (delay1.getNumberValue() > 0)
                                return;
                        } else if (!isBestAxe(Minecraft.player.inventoryContainer.getSlot(axeSlot).getStack())) {
                            swap(i, axeSlot - 36);
                            timer.reset();
                            if (delay1.getNumberValue() > 0)
                                return;
