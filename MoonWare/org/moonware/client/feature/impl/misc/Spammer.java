...
public class Spammer extends Feature {
    ...
    public Spammer() {
        super("ChatSpammer", "Автоматически спамит сообщениями в чат", Type.Other);
    ...
    private List<EntityPlayer> getPlayerByTab() {
        ...
        for (NetworkPlayerInfo info : Minecraft.player.connection.getPlayerInfoMap()) {
            ...
            list.add(Minecraft.world.getPlayerEntityByName(info.getGameProfile().getName()));
        }
        return list;
    }
    ...
    public void onUpdate(EventUpdate event) throws IOException {
        ...
        setSuffix(mode + ", " + (int) delay.getNumberValue());
        if (mode.equalsIgnoreCase("Default")) {
            try {
                ...
                if (ticks++ % (int) delay.getNumberValue() == 0) {
                    Minecraft.player.sendChatMessage("![" + str1 + "]" + "`vk.com/moonwarerage ` " + "[" + str2 + "]");
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            if (ticks++ % (int) delay.getNumberValue() == 0) {
                switch (counter) {
                    case 0:
                        Minecraft.player.sendChatMessage(str3 + "!Твой клиент зaлупa ебaная)) Кид@й мнe дyэль: \"/duel " + Minecraft.player.getName() + "\".  Карта: Пляж " + " [" + str1 + "]" + " [" + str2 + "]");
                        counter++;
                        break;
                    case 1:
                        Minecraft.player.sendChatMessage(str3 + "!Правда думаешь твой клиент лучше?) Кидaй мне дуэль: \"/duel " + Minecraft.player.getName() + "\". Карта: Пляж " + " [" + str1 + "]" + " [" + str2 + "]");
                        counter++;
                        break;
                    case 2:
                        Minecraft.player.sendChatMessage(str3 + "!Ты как себя ведешь бл9дина eбaнaя? Кiдай мне дуэль: \"/duel " + Minecraft.player.getName() + "\".  Карта: Пляж " + " [" + str1 + "]" + " [" + str2 + "]");
                        counter = 0;
                        break;
                }
            }
            ...
            String str1 = RandomStringUtils.randomAlphabetic(3);
            String str2 = RandomStringUtils.randomPrint(5);
            File file = new File(Minecraft.gameDir + "\\moonware", "spammer.txt");
            if (!file.exists()) {
                file.createNewFile();
            }
                ...
                if (ticks++ % (int) delay.getNumberValue() == 0) {
                    if (counter == 0) {
                        if (randomSymbols.getBoolValue()) {
                            Minecraft.player.sendChatMessage("! '" + scanner.nextLine() + "' " + str2 + str1);
                        } else {
                            Minecraft.player.sendChatMessage(scanner.nextLine());
                        }
                        counter = 0;
                    }
                }
            ...
            for (EntityPlayer e : getPlayerByTab()) {
                if (e == null)
                    continue;
                if (ticks++ % (int) delay.getNumberValue() == 0) {
                    if (e != Minecraft.player) {
                        Minecraft.player.sendChatMessage("/msg " + e.getName() + " vk.com/sakurashop1337 лучший чит, быстрее покупай!");
                    }
                }
            }
        }
    ...
    public void onEnable() {
        ...
        if (mode.equalsIgnoreCase("Custom")) {
            Sys.openURL(Minecraft.gameDir + "\\moonware\\spammer.txt");
        }
        super.onEnable();
    }
}