...
public class BullingBot extends Feature {
    ...
    public BullingBot() {
        super("BullingBot", "При ударе игрока отправляет буллинг сообщение в его адресс", Type.Other);
    ...
    public void onPacket(EventAttackClient event) {
        if (event.getEntity() instanceof EntityPlayer) {
            ...
            if (timer.hasReached(delay.getNumberValue() * 1000) && !lastMessage.equals(text) || lastMessage == null) {
                Minecraft.player.sendChatMessage("!" + "[" + RandomStringUtils.randomAlphanumeric(5) + "] " + entity.getName() + " " + text + " " + "[" + RandomStringUtils.randomAlphanumeric(5) + "]");
                lastMessage = text;
                timer.reset();
            }
        }
