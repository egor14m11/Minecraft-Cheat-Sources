...
public class AutoArmor extends Feature {
    ...
    public void onUpdate(EventUpdate event) {
        ...
        setSuffix("" + delay.getNumberValue());

        if (!(Minecraft.screen instanceof GuiInventory) && (openInventory.getBoolValue()))
            return;

        if (Minecraft.screen instanceof GuiContainer && !(Minecraft.screen instanceof InventoryEffectRenderer)) {
            return;
        }
        ...
        for (int i : types) {
            ...
            if (timerUtils.hasReached(delay.getNumberValue() * 100)) {
                if (!isNullOrEmpty(oldArmor)) {
                    Minecraft.playerController.windowClick(0, 8 - i, 0, ClickType.QUICK_MOVE, Minecraft.player);
                }
                Minecraft.playerController.windowClick(0, j, 0, ClickType.QUICK_MOVE, Minecraft.player);
                timerUtils.reset();
            }
            break;
        }
