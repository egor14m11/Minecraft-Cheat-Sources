...
public class HitFlyingParticles
        extends Module {
    ArrayList<partical> particals = new ArrayList();
    public static ColorSetting color = new ColorSetting("Particles Color", new Color(255, 255, 255).getRGB());
    public static SliderSetting deleteAfter = new SliderSetting("Delete", 1500.0f, 100.0f, 10000.0f, 1.0f);

    public HitFlyingParticles() {
        super("HitFlyingParticles", Categories.Render);
        this.addSetting(color, deleteAfter);
    }
            ...
            for (Entity entity : mc.world.loadedEntityList) {
                if (entity != null && (entity instanceof EntityPlayer)) {
                    ...
                    for (int i = 0; i < this.particals.size(); ++i) {
                        if (System.currentTimeMillis() - this.particals.get(i).getTime() < (long)deleteAfter.value) continue;
    ...
    public void render(EventRender3D event) {
        if (Minecraft.getMinecraft().player != null && mc.world != null) {
            for (partical partical2 : this.particals) {
                ...
                Color colorc = new Color(color.color);
