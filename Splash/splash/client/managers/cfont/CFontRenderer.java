package splash.client.managers.cfont;
...
import splash.client.cfont.CFont;
...
public class CFontRenderer extends CFont {
    ...
    protected CharData[] boldChars = new CFont.CharData[256];
    ...
    public void updateFontType() {
        FontType type = (FontType) Splash.getInstance().getValueManager().getValue(Splash.getInstance().getModuleManager().getModuleByClass(UI.class), "Font").getValue();
        this.fontType = type;
    }
    ...
    public void drawStringWithShadow(String text, float x, float y, int color) {
        switch(getFontType()) {
            ...
            case CUSTOM: {
                drawStringWithShadowFake(text, x, y, color);
                break;
            }
        }
    }
    ...
    public void drawCenteredStringWithShadow(String text, float x, float y, int color) {
        switch(getFontType()) {
            case MINEMAN: {
                getMinecraftFontRenderer().drawCenteredString(text, (int)x, (int)y, color);
                break;
            }
            case CUSTOM: {
                drawCenteredStringWithShadowFake(text, x, y, color);
                break;
            }
        }
    }
    ...
    public void drawCenteredString(String text, float x, float y, int color) {
        switch(getFontType()) {
            ...
            case CUSTOM: {
                drawCenteredStringFake(text, x, y, color);
                break;
            }
        }
    }

    public int getStringWidthCustom(String text) {
    ...
    public int getFontHeightCustom() {
        if(getFontType() == FontType.CUSTOM) {
            return getHeight();
        }
        if(getFontType() == FontType.MINEMAN) {
    ...
    protected float drawStringWithShadowFake(String text, double x, double y, int color) {
            float shadowWidth = drawStringFake(text, x + 1D, y + 1D, color, true);
            return Math.max(shadowWidth, drawStringFake(text, x, y, color, false));
    }

    protected float drawStringFake(String text, float x, float y, int color) {
        return drawStringFake(text, x, y, color, false);
    }

    protected float drawCenteredStringWithShadowFake(String text, float x, float y, int color) {
        return drawStringWithShadowFake(text, x - getStringWidth(text) / 2, y, color);
    }

    protected float drawCenteredStringFake(String text, float x, float y, int color) {
        return drawStringFake(text, x - getStringWidth(text) / 2, y, color);
    }

    protected float drawStringFake(String text, double x, double y, int color, boolean shadow) {
        x -= 1;
        y -= 2;
        if (text == null) {
            return 0.0F;
