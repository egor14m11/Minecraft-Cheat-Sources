...
public class Scaffold extends Module {
    ...
    public void onDisable() {
    	if (!switcher.getValue() && mc.thePlayer.inventory.currentItem != item) {
			mc.thePlayer.sendQueue.addToSendQueue(new C09PacketHeldItemChange(mc.thePlayer.inventory.currentItem = item));
    	}
    	super.onDisable();
            ...
            if (tower.getValue().equals(true)) {
                if (mc.gameSettings.keyBindJump.isKeyDown() && !mc.thePlayer.isPotionActive(Potion.jump)) {
                    if (!mc.thePlayer.isMoving()) {
                        mc.thePlayer.motionY = mc.thePlayer.ticksExisted % 8 == 0 ? -.02f : 0.42F;
    ...
    public void setYaw(EventPlayerUpdate e, float yaw) {
    	e.setYaw(yaw);
    }
