...
public abstract class EntityLivingBase extends Entity
{
    ...
    private int getArmSwingAnimationEnd()
    {
        ...
        return this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : animations.swingSpeed.getValue().intValue());
    }
    ...
    protected void jump()
    {
    	PlayerJumpEvent event = new PlayerJumpEvent((double)this.getJumpUpwardsMotion());
    	Splash.INSTANCE.getEventBus().publish(event);
    	if (event.isCancellled()) return;
