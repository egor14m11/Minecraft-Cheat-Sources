...
import splash.client.modules.visual.Animations.Mode;
...
public class ItemRenderer {
	...
	private void func_178095_a(AbstractClientPlayer clientPlayer, float p_178095_2_, float p_178095_3_) {
		...
		this.mc.getTextureManager().bindTexture(clientPlayer.getLocationSkin());
		GlStateManager.translate(-1.0F, 3.7F, 3.5F);
	...
	private void transformFirstPersonItem(float equipProgress, float swingProgress) {
		Animations animations = (Animations) Splash.getInstance().getModuleManager().getModuleByClass(Animations.class);
		if (!animations.farSword.getValue())  {
		
		GlStateManager.translate(0.51F, -0.47F, -0.71999997F);
		GlStateManager.translate(0.0F, equipProgress * -0.6F, 0.0F);
		GlStateManager.rotate(45.0F, 0.0F, 1.0F, 0.0F);
		}else  {
			GlStateManager.translate(0.51F, -0.47F, -0.89999997F);
			GlStateManager.translate(0.0F, equipProgress * -0.6F, 0.0F);
	...
	public void renderItemInFirstPerson(float partialTicks) {
	      ...
	      boolean block = aura.isModuleActive() &&  aura.blocking && aura.targetIndex != -1 && aura.targetList.get(aura.targetIndex) != null && aura.targetList.get(aura.targetIndex).getDistanceToEntity(mc.thePlayer) <= aura.targetingDist.getValue();
	      ...
	      if (this.itemToRender != null) {
	         if (this.itemToRender.getItem() == Items.filled_map) {
	            ...
	            case BLOCK: 				
	               ...
	               if (Splash.INSTANCE.getModuleManager().getModuleByClass(Animations.class).isModuleActive()) { 
	                  switch(((Animations)Splash.INSTANCE.getModuleManager().getModuleByClass(Animations.class)).mode.getValue()) {
	                  default:
	                	  break;
	                  case DEFAULT:
	                  ...
	                  case SPLASH:
							...
							GlStateManager.rotate(-var9 * 40.0F / 0.8F, -.4F, -0.0F, 6.0F);
							GlStateManager.rotate(-var9 * 20.0F, 1.5F, -1.1F, -0.8F); 
							break;
...
/*		
        if (this.itemToRender != null) {
            ...
            } else if (block && sword || abstractclientplayer.getItemInUseCount() > 0) {

				if (Splash.INSTANCE.getModuleManager().getModuleByClass(Animations.class).isModuleActive() &&  mc.thePlayer.getCurrentEquippedItem().getItem() instanceof ItemSword) {

					Mode mode = ((Animations)Splash.INSTANCE.getModuleManager().getModuleByClass(Animations.class)).mode.getValue();

					
					if (mode.equals(Mode.EPIC)) {
						
					}
					if (mode.equals(Mode.PUNCH)) {

					}
					if (mode.equals(Mode.SIGMA)) {
					}
					if (mode.equals(Mode.HELIUM)) {
					}
					if (mode.equals(Mode.POKE)) {


					}
					if (mode.equals(Mode.KANSIO)) {


					}
					if (mode.equals(Mode.SPIN)) {

					}
					if (mode.equals(Mode.DEFAULT)) {

					}
					if (mode.equals(Mode.OHARE)) {


					}
					if (mode.equals(Mode.SLIDE)) {
                    ...
                    this.func_178103_d();
                    GL11.glTranslatef(0.4f, 0.1f, -0.01f);
                    final float var19 = MathHelper.sin(f2 * f2 * 5.1415925f);
                    GlStateManager.translate(-0.1f, 0.3f, 0.2f);
                    GlStateManager.rotate(var19 * 13.0f, -10.0f, -1.4f, -10.0f);
                }
            }
