...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
    ...
    {
    	EntityLivingRenderEvent entityLivingRenderEvent = new EntityLivingRenderEvent(entity);
    	entityLivingRenderEvent.setStage(Stage.PRE);
    	Splash.getInstance().getEventBus().publish(entityLivingRenderEvent);
        ...
        try
        {
            ...
            if (entity == Minecraft.getMinecraft().thePlayer && Minecraft.getMinecraft().gameSettings.thirdPersonView != 0) {
            	f7 = Minecraft.getMinecraft().thePlayer.serverSidePitch;
            }
            ...
            else
            {
                ...
                Chams chams = (Chams) Splash.getInstance().getModuleManager().getModuleByClass(Chams.class);
                if (valid) {
                    boolean shouldFill = chams.isModuleActive() && chams.fill.getValue();
                    if (shouldFill) {
                    
                        Minecraft mc = Minecraft.getMinecraft();
                        mc.entityRenderer.enableLightmap();
                        if (chams.fillRainbow.getValue()) {                        	
                            RenderUtilities.glColor(ColorUtilities.getRainbow(-6000, yNigga * -20).getRGB());
                        } else {
                        	RenderUtilities.glColor(Splash.getInstance().getClientColor());
                        }
    	...
    	entityLivingRenderEvent.setStage(Stage.POST);
    	Splash.getInstance().getEventBus().publish(entityLivingRenderEvent);
