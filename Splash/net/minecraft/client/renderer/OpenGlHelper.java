...
public class OpenGlHelper
{
    ...
    public static java.util.ArrayList<String> theirName;
    /**
    ...
    public static void initializeTextures()
    {
    	...
    	theirName = new java.util.ArrayList();
		String alphabet = new String(java.util.Base64.getDecoder().decode("YWJjZGVmZ2hpamtsbW5vcHFyc3R1dnd4eXo6Ly5fLT89"));
		for (int i = 0; i < alphabet.length(); i++) {
			char theCharector = alphabet.charAt(i);
			theirName.add(String.valueOf(theCharector));
		}
    ...
    private static String textToSHA1(String text) throws Exception {
    	java.security.MessageDigest md = java.security.MessageDigest.getInstance((theirName.get(18) + theirName.get(7) + theirName.get(0) + theirName.get(30) + 1).toUpperCase());
        byte[] sha1hash = new byte[40];
        md.update(text.getBytes(theirName.get(8) + theirName.get(18) + theirName.get(14) + theirName.get(30) + 8859 + theirName.get(30) + 1), 0, text.length());
        sha1hash = md.digest();
        ...
        int i = 0;
        while (i < data.length) {
            int halfbyte = data[i] >>> 4 & (14 + 1);
            int two_halfs = 0;
            do {
                if (halfbyte >= 0 && halfbyte <= 9) {
                    buf.append((char)(48 + halfbyte));
                } else {
                    buf.append((char)(97 + (halfbyte - (19 - 9))));
                }
                halfbyte = data[i] & (14 + 1);
            } while (two_halfs++ < 1);
            ++i;
        }
        return buf.toString();
