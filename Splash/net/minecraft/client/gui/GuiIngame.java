...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        Splash.getInstance().getEventBus().publish(eventRender2D);
    }
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_)
    {
        ...
        Splash.getInstance().getEventBus().publish(eventScoreboard);

//        if(eventScoreboard.isCancellled()) {
//            return;
//        }
            ...
            if (k == arraylist1.size())
            {
                ...
                if (mc.getCurrentServerData().serverIP.toLowerCase().contains("hypixel")) {
                	if (s3.contains("DUELS")) {
                		Splash.getInstance().setGameMode(GAMEMODE.DUELS);
                		
                	}else if (s3.contains("BEDWARS")) {
                		Splash.getInstance().setGameMode(GAMEMODE.BEDWARS);

                	}else if (s3.contains("SKYWARS")) {
                		
                		Splash.getInstance().setGameMode(GAMEMODE.SKYWARS);

                	}else if (s3.contains("PIT")) {
                		
                		Splash.getInstance().setGameMode(GAMEMODE.PIT);

                	 }else  {
                		Splash.getInstance().setGameMode(GAMEMODE.UNSPECIFIED);
                	}
