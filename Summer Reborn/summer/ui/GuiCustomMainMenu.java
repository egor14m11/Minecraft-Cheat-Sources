...
public class GuiCustomMainMenu extends GuiScreen implements GuiYesNoCallback {
    ...
    public void initGui() {
    	if(!Login.loggedin) {
            String[] displayNames = new String[]{"Singleplayer", "Multiplayer", "Options", "Alt Manager", "Changelog", "Exit", "Login"};
...
//        RenderUtils.drawRect(0, 0, width, height, Color.BLACK.getRGB());
        ...
        if(!Login.loggedin) {
        Gui.drawGradientRect(width / 2 - 80, height / 2 - 72, width / 2 + 110, height / 2  + (5 * 29), new Color(10, 10, 10, 255).getRGB(), new Color(10, 10, 10, 255).getRGB());
        Gui.drawGradientRect(width / 2 - 79, height / 2 - 71, width / 2 + 109, height / 2 - 1 + (5 * 29), new Color(59, 59, 59, 255).getRGB(), new Color(59, 59, 59, 255).getRGB());
//        Gui.drawGradientRect(width / 2 - 76, height / 2 - 69, width / 2 + 106, height / 2 - 28 + (5 * 23), new Color(74, 74, 74, 255).getRGB(), new Color(74, 74, 74, 255).getRGB());
        Gui.drawGradientRect(width / 2 - 78, height / 2 - 70, width / 2 + 108, height / 2 - 2 + (5 * 29), new Color(20, 20, 20, 255).getRGB(), new Color(10, 10, 10, 255).getRGB());
        }else {
...
//            Gui.drawGradientRect(width / 2 - 76, height / 2 - 69, width / 2 + 106, height / 2 - 28 + (5 * 23), new Color(74, 74, 74, 255).getRGB(), new Color(74, 74, 74, 255).getRGB());
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        if (button.id == 6) {
            this.mc.displayGuiScreen(new Login(null));
        }
