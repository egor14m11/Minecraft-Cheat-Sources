...
public class ESP extends Cheats {
    ...
    private final Setting arrows;
    private final Setting skeletal;
    private final Setting arrowSize;
    private final Setting arrowRadius;
    private final Setting arrowHue;
    ...
    public ESP() {
        ...
        Summer.INSTANCE.settingsManager.Property(arrows = new Setting("FOV Arrows", this, false));
        Summer.INSTANCE.settingsManager.Property(arrowHue = new Setting("Arrow Hue", this, 0.8, 0, 1.0, false, arrows::getValBoolean));
        Summer.INSTANCE.settingsManager.Property(arrowSize = new Setting("Arrow Size", this, 7, 3, 30, false, arrows::getValBoolean));
        Summer.INSTANCE.settingsManager.Property(arrowRadius = new Setting("Arrow Radius", this, 70, 10, 150, false, arrows::getValBoolean));
    ...
    public void onRender2DEvent(EventRender2D event) {
        for (EntityPlayer player : entityPosMap.keySet()) {
            ...
            if (arrows.getValBoolean()) {
                glPushMatrix();
                float arrowSizes = arrowSize.getValFloat();
                float alpha = Math.max(1.0F - (Minecraft.thePlayer.getDistanceToEntity(player) / 30.0F), 0.2F);
                Color c = Color.getHSBColor(arrowHue.getValFloat(), 1.0F, 1.0F);
                Color c2 = Color.getHSBColor(nametagsHue.getValFloat(), 1.0F, 1.0F);
                int color = c.getRGB();
                int colorFriend = c2.getRGB();
                glTranslatef(middleX + 0.5F, middleY - (arrowSizes / 2.0F), 1.0F);
                float yaw = RenderUtils.interpolate(
                        RotationUtils.getYawToEntity(player, true),
                        RotationUtils.getYawToEntity(player, false),
                        event.getTicks()) -
                        RenderUtils.interpolate(Minecraft.thePlayer.prevRotationYaw, Minecraft.thePlayer.rotationYaw, event.getTicks());
                glRotatef(yaw, 0, 0, 1);
                glTranslatef(0.0F, -arrowRadius.getValFloat(), 0.0F);
                ...
                if (Summer.INSTANCE.friendManager.isFriend(player.getName()) && friendColors.getValBoolean()) {
                    glColor4ub(
                            (byte) (colorFriend >> 16 & 255),
                            (byte) (colorFriend >> 8 & 255),
                            (byte) (colorFriend & 255),
                            (byte) (colorFriend * 255));
                } else
                    glColor4ub(
                            (byte) (color >> 16 & 255),
