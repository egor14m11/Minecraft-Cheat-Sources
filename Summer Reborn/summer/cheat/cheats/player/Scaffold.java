...
//        Summer.instance.sm.Property(extend = new Setting("Extend", this, 0.1, 0.1, 6, false));
    ...
    private BlockCache grab() {
        ...
        if (down && downwardsTimer.hasReached(35 / Timer.timerSpeed) && Minecraft.thePlayer.isMoving()
                && downWards.getValBoolean()) {
            position = new BlockPos(new BlockPos(Minecraft.thePlayer.getPositionVector().xCoord,
                    Minecraft.thePlayer.getPositionVector().yCoord - 0.5, Minecraft.thePlayer.getPositionVector().zCoord))
                    .offset(EnumFacing.DOWN);
            downwardsTimer.reset();
        } else if (Minecraft.thePlayer.isMoving() && !mc.gameSettings.keyBindJump.getIsKeyPressed()) {
