...
public class Flight extends Cheats {
    ...
    //	mc.timer.timerSpeed = 2f;
        ...
        Minecraft.thePlayer.cameraYaw = 0.09f;
        ...
        if (damaged && timer.hasReached(600)) {
            if (zoom.getValBoolean()) {
...
//         if (timer.hasReached(100)){ mc.thePlayer.jump(); timer.reset(); }
    ...
    public void damagePlayer() {
        // Old
        /*
         * for (int i = 0; i <= (mc.thePlayer.isPotionActive(Potion.jump) ? 140 : 70);
         * ++i) { mc.thePlayer.sendQueue.getNetworkManager().sendPacket(new
         * C03PacketPlayer.C06PacketPlayerPosLook(mc.thePlayer.posX, mc.thePlayer.posY +
         * 0.05, mc.thePlayer.posZ, mc.thePlayer.rotationYaw,
         * mc.thePlayer.rotationPitch, false));
         * mc.thePlayer.sendQueue.getNetworkManager().sendPacket(new
         * C03PacketPlayer.C06PacketPlayerPosLook(mc.thePlayer.posX, mc.thePlayer.posY,
         * mc.thePlayer.posZ, mc.thePlayer.rotationYaw, mc.thePlayer.rotationPitch,
         * false)); mc.thePlayer.sendQueue.getNetworkManager().sendPacket(new
         * C03PacketPlayer.C06PacketPlayerPosLook(mc.thePlayer.posX, mc.thePlayer.posY +
         * 1.0E-5, mc.thePlayer.posZ, mc.thePlayer.rotationYaw,
         * mc.thePlayer.rotationPitch, false)); }
         * mc.thePlayer.sendQueue.getNetworkManager().sendPacket(new
         * C03PacketPlayer.C06PacketPlayerPosLook(mc.thePlayer.posX, mc.thePlayer.posY,
         * mc.thePlayer.posZ, mc.thePlayer.rotationYaw, mc.thePlayer.rotationPitch,
        ...
        for (int i = 0; i < MoveUtils.getMinFallDist() / 0.050F + 1.0; ++i) {
            netHandler.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(x, y + 0.06F, z, false));
            netHandler.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(x, y + (float) 5.0E-4D, z, false));
            netHandler.addToSendQueue(
                    new C03PacketPlayer.C04PacketPlayerPosition(x, y + 0.006F + (float) 6.01E-8D, z, false));
        }
    ...
    public static void sendPackets() {
        ...
        packets.clear();
        Minecraft.thePlayer.setPosition(Minecraft.thePlayer.posX, Minecraft.thePlayer.posY + yPos, Minecraft.thePlayer.posZ);
    }
}
