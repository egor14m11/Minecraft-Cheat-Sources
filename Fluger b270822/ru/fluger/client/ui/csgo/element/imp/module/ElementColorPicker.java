...
extends Element {
    ...
    public ElementColorPicker(ElementSettings parent, ColorSetting colorProperty) {
    ...
    public void render(int mouseX, int mouseY) {
        this.width = this.settings.getWidth() - 10.0;
        this.height = 10.0;
        ...
        this.mc.smallfontRenderer.drawStringWithShadow(this.colorProperty.getName(), x + 5.0f, y + height / 2.0f - 2.0f, textColor);
        ...
        if (this.extended) {
            ...
            while ((float)i < times) {
                ...
                RectHelper.drawGradientRect(sLeft + 0.5f, sY, sRight + 1.0f, sY + size, Color.HSBtoRGB(inc * (float)i, 1.0f, 1.0f), Color.HSBtoRGB(inc * (float)(i + 1), 1.0f, 1.0f));
                if (!last) {
                    sY += size;
                }
                ++i;
            ...
            RectHelper.drawGradientRect(sLeft + 0.5f, sTop + 0.5f, sRight + 1.0f, sBottom - 0.5f, new Color(r, g, b, 50).getRGB(), new Color(r, g, b, 255).getRGB());
    ...
    private void drawCheckeredBackground(float x, float y, float right, float bottom) {
        RectHelper.drawRect((double)x, (double)y, (double)right, (double)bottom, -1);

        for(boolean off = false; y < bottom; ++y) {
           for(float x1 = x + (float)((off = !off) ? 1 : 0); x1 < right; x1 += 2.0F) {
              RectHelper.drawRect((double)x1, (double)y, (double)(x1 + 1.0F), (double)(y + 1.0F), -8355712);
           }
        }
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        if (this.collided(mouseX, mouseY) && button == 1) {
            boolean bl = this.extended = !this.extended;
        }
        if (this.extended && button == 0) {
            float x = (float)this.getX();
            float y = (float)this.getY();
            float cpLeft = x - 2.0f;
            float cpTop = (float)((double)y + this.getHeight() - 2.0);
            float cpRight = x + 80.0f + 2.0f;
            float cpBottom = (float)((double)y + this.getHeight() + 80.0 + 2.0);
            float sLeft = x + 80.0f - 1.0f;
            float sTop = (float)((double)y + this.getHeight() + 2.0);
            float sRight = sLeft + 10.0f;
            float sBottom = (float)((double)y + this.getHeight() + 80.0 - 2.0);
            float asLeft = x + 80.0f + 6.0f;
            float asTop = (float)((double)y + this.getHeight() + 2.0);
            float asRight = asLeft + 10.0f;
            float asBottom = (float)((double)y + this.getHeight() + 80.0 - 2.0);
    ...
    public int getHeightWithExpand() {
        return (int)(this.getHeight() + 80.0);
    }
}
