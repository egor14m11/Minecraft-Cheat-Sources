...
implements PropertyComponent {
    ...
    public void drawComponent(ScaledResolution scaledResolution, int mouseX, int mouseY) {
        ...
        if (this.doubleProperty.getName().equalsIgnoreCase("Rotation Speed") && this.doubleProperty.getDesc().startsWith("\ufffd\ufffd\ufffd\ufffd\ufffd\ufffd\ufffd\ufffd") && this.doubleProperty.getCurrentValue() >= 5.0f) {
        ...
        RectHelper.drawBorderedRect(x + 3.0f, y + height - 5.0f + -1.0f, x + (width - 3.0f), y + height - 1.0f, new Color(40, 39, 39).getRGB(), 2.0);
        RectHelper.drawGradientRect(x + 4.0f, y + height - 5.0f, x + width * this.currentValueAnimate - 4.0f, y + height - 2.0f, color, new Color(color).brighter().getRGB());
        ...
        if (hovered) {
            if (this.doubleProperty.getDesc() != null && this.descTimer.hasReached(250.0)) {
                RenderHelper.drawBlurredShadow(x + width + 20.0f, y + height / 1.5f - 5.0f, 5 + SliderPropertyComponent.mc.fontRenderer.getStringWidth(this.doubleProperty.getDesc()), 10.0f, 20, new Color(0, 0, 0, 180));
                RectHelper.drawSmoothRect(x + width + 20.0f, y + height / 1.5f + 4.5f, x + width + 25.0f + (float)SliderPropertyComponent.mc.fontRenderer.getStringWidth(this.doubleProperty.getDesc()), y + 6.5f, new Color(0, 0, 0, 80).getRGB());
