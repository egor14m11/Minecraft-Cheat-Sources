...
implements Helper {
    ...
    public static void blurAreaBoarder(float x, float f, float width, float height, float intensity, float blurWidth, float blurHeight) {
        ...
        GL11.glScissor((int)((int)(x * (float)factor)), (int)((int)((float)RenderHelper.mc.displayHeight - f * (float)factor - height * (float)factor) + 1), (int)((int)(width * (float)factor)), (int)((int)(height * (float)factor)));
    ...
    public static void blurAreaBoarder(int x, int y, int width, int height, float intensity) {
        ScaledResolution scale = new ScaledResolution(mc);
        int factor = Fluger.scale.getScale();
        int factor2 = Fluger.scale.calc(scale.getScaledWidth());
        int factor3 = Fluger.scale.calc(scale.getScaledHeight());
        if (lastScale != factor || lastScaleWidth != factor2 || lastScaleHeight != factor3 || buffer == null || blurShader == null) {
            RenderHelper.inShaderFBO();
        }
        lastScale = factor;
        lastScaleWidth = factor2;
        lastScaleHeight = factor3;
        GL11.glScissor((int)(x * factor), (int)(RenderHelper.mc.displayHeight - y * factor - height * factor), (int)(width * factor), (int)(height * factor));
        GL11.glEnable((int)3089);
    ...
    public static void scissorRectScale(float x, float y, float width, double height) {
        ScaledResolution sr = new ScaledResolution(mc);
        int factor = Fluger.scale.getScale();
        GL11.glScissor((int)((int)(x * (float)factor)), (int)((int)(((double)Fluger.scale.calc(sr.getScaledHeight()) - height) * (double)factor)), (int)((int)((width - x) * (float)factor)), (int)((int)((height - (double)y) * (double)factor)));
    }
    ...
    public static void blurSpot(int x, int y, int width, int height, int blurRadius, int iterations) {
        ...
        if (blurSpotCache.containsKey(identifier)) {
            ...
            ByteBuffer buffer = BufferUtils.createByteBuffer((int)(width * height * bpp)).order(ByteOrder.nativeOrder());
            GL11.glReadPixels((int)x, (int)(RenderHelper.mc.displayHeight - y - height), (int)width, (int)height, (int)6407, (int)5121, (ByteBuffer)buffer);
    ...
    public static void drawSphere(double x, double y, double z, float size, int slices, int stacks) {
        ...
        GL11.glTranslated((double)(x - RenderHelper.mc.getRenderManager().renderPosX), (double)(y - RenderHelper.mc.getRenderManager().renderPosY), (double)(z - RenderHelper.mc.getRenderManager().renderPosZ));
    ...
    public static void drawArrow(float x, float y, float scale, float width, boolean up, int hexColor) {
        ...
        GL11.glLineWidth((float)width);
        GL11.glBegin((int)1);
        GL11.glVertex2d((double)(x /= scale), (double)((y /= scale) + (float)(up ? 4 : 0)));
    ...
    public static void drawCylinder(float radius, float height, int segments, boolean flag) {
        ...
        GL11.glBegin((int)9);
        for (i = 0; i < segments; ++i) {
            GL11.glVertex3f((float)vertecesTop[segments - 1 - i][0], (float)vertecesTop[segments - 1 - i][1], (float)vertecesTop[segments - 1 - i][2]);
        }
        GL11.glEnd();
        GL11.glBegin((int)9);
        for (i = 0; i < segments; ++i) {
            GL11.glVertex3f((float)vertecesBottom[i][0], (float)vertecesBottom[i][1], (float)vertecesBottom[i][2]);
        }
        GL11.glEnd();
        for (i = 0; i < segments; ++i) {
            GL11.glBegin((int)7);
            GL11.glVertex3f((float)vertecesTop[i][0], (float)vertecesTop[i][1], (float)vertecesTop[i][2]);
            GL11.glVertex3f((float)vertecesTop[(i + 1) % segments][0], (float)vertecesTop[(i + 1) % segments][1], (float)vertecesTop[(i + 1) % segments][2]);
            GL11.glVertex3f((float)vertecesBottom[(i + 1) % segments][0], (float)vertecesBottom[(i + 1) % segments][1], (float)vertecesBottom[(i + 1) % segments][2]);
            GL11.glVertex3f((float)vertecesBottom[i][0], (float)vertecesBottom[i][1], (float)vertecesBottom[i][2]);
            GL11.glEnd();
        }
        GL11.glEnable((int)3553);
    ...
    public static void drawCone(float radius, float height, int segments, boolean flag) {
        ...
        GL11.glBegin((int)9);
        for (i = 0; i < segments; ++i) {
            GL11.glVertex3f((float)verteces[i][0], (float)verteces[i][1], (float)verteces[i][2]);
        }
        GL11.glEnd();
        for (i = 0; i < segments; ++i) {
            GL11.glBegin((int)4);
            GL11.glVertex3f((float)verteces[i][0], (float)verteces[i][1], (float)verteces[i][2]);
            GL11.glVertex3f((float)topVertex[0], (float)topVertex[1], (float)topVertex[2]);
            GL11.glVertex3f((float)verteces[(i + 1) % segments][0], (float)verteces[(i + 1) % segments][1], (float)verteces[(i + 1) % segments][2]);
            GL11.glEnd();
        }
        GL11.glEnable((int)3553);
        GL11.glEnable((int)2884);
    ...
    private static Vector4f getTransformedMatrix(Vec3d posIn) {
        ...
        Vector4f vector4f = new Vector4f((float)relativePos.xCoord, (float)relativePos.yCoord, (float)relativePos.zCoord, 1.0f);
    ...
    public static void drawTriangle(float x, float y, float width, float height, int firstColor, int secondColor) {
        GL11.glPushMatrix();
        GL11.glDisable((int)3553);
        RenderHelper.enableSmoothLine(1.0f);
        GL11.glBegin((int)9);
        RenderHelper.color(firstColor, 1.0f);
        GL11.glVertex2f((float)x, (float)(y - 2.0f));
        GL11.glVertex2f((float)(x + width), (float)(y + height));
        GL11.glVertex2f((float)(x + width), (float)y);
        GL11.glVertex2f((float)x, (float)(y - 2.0f));
        GL11.glEnd();
        GL11.glBegin((int)9);
        RenderHelper.color(secondColor, 1.0f);
        GL11.glVertex2f((float)(x + width), (float)y);
        GL11.glVertex2f((float)(x + width), (float)(y + height));
        GL11.glVertex2f((float)(x + width * 2.0f), (float)(y - 2.0f));
        GL11.glVertex2f((float)(x + width), (float)y);
        GL11.glEnd();
        GL11.glBegin((int)3);
        RenderHelper.color(firstColor, 1.0f);
        ...
        GL11.glVertex2f((float)(x + width), (float)y);
        GL11.glEnd();
        RenderHelper.disableSmoothLine();
        GL11.glEnable((int)3553);
        GL11.glPopMatrix();
    }
    ...
    public static void drawTriangle(float x, float y, float size, float vector, int color) {
        ...
        GL11.glRotatef((float)(180.0f + vector), (float)0.0f, (float)0.0f, (float)1.0f);
        float alpha = (float)(color >> 24 & 0xFF) / 255.0f;
        float red = (float)(color >> 16 & 0xFF) / 255.0f;
        float green = (float)(color >> 8 & 0xFF) / 255.0f;
        float blue = (float)(color & 0xFF) / 255.0f;
        ...
        GL11.glLineWidth((float)1.0f);
        GL11.glBegin((int)6);
        GL11.glVertex2d((double)0.0, (double)size);
