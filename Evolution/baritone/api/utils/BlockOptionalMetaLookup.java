...
public class BlockOptionalMetaLookup {
	...
	public BlockOptionalMetaLookup(Block... blocks) {
		this.boms = Stream.of(blocks).map(BlockOptionalMeta::new).toArray(BlockOptionalMeta[]::new);
	}

	public BlockOptionalMetaLookup(List<Block> blocks) {
		this.boms = blocks.stream().map(BlockOptionalMeta::new).toArray(BlockOptionalMeta[]::new);
	}
	...
	public boolean has(ItemStack stack) {
		for (BlockOptionalMeta bom : boms) {
			if (Block.getBlockFromItem(stack.getItem()) == bom.getBlock()) {
				return true;
			}
		}
