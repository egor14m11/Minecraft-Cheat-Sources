    ...
    public ImageData filter(Effect effect, RenderState renderState, BaseTransform baseTransform, Rectangle rectangle, ImageData ... arrimageData) {
        ...
        for (int i = 0; i < 0 + n12; ++i) {
            ...
            for (int j = 0; j < 0 + n11; ++j) {
                ...
                float f16 = (float)(n16 >> 16 & 0xFF) / 255.0f;
                float f17 = (float)(n16 >> 8 & 0xFF) / 255.0f;
                float f18 = (float)(n16 & 0xFF) / 255.0f;
                float f19 = (float)(n16 >>> 24) / 255.0f;
                f14 = f16;
                f15 = f17;
                float f20 = f18;
                float f21 = f19;
                ...
                f16 = (float)(n15 >> 16 & 0xFF) / 255.0f;
                f17 = (float)(n15 >> 8 & 0xFF) / 255.0f;
                f18 = (float)(n15 & 0xFF) / 255.0f;
                f19 = (float)(n15 >>> 24) / 255.0f;
                f22 = f16 * f;
                f23 = f17 * f;
                float f24 = f18 * f;
                float f25 = f19 * f;
                ...
                float f40 = f32 / f33;
                float f41 = f35;
                f41 = f12 = (float)Math.sqrt(f41);
                float f42 = f35;
                float f43 = f41;
                float f44 = f42 <= 0.25f ? ((16.0f * f42 - 12.0f) * f42 + 4.0f) * f42 : f43;
                f42 = f11 = f44;
                f43 = f36;
                f41 = f12 = (float)Math.sqrt(f43);
                f43 = f36;
                f44 = f41;
                float f45 = f43 <= 0.25f ? ((16.0f * f43 - 12.0f) * f43 + 4.0f) * f43 : f44;
                f43 = f11 = f45;
                f44 = f37;
                f41 = f12 = (float)Math.sqrt(f44);
                f44 = f37;
                f45 = f41;
                float f46 = f44 <= 0.25f ? ((16.0f * f44 - 12.0f) * f44 + 4.0f) * f44 : f45;
                f44 = f11 = f46;
                float f47 = f29 == 0.0f ? f30 : (f33 == 0.0f ? f26 : (f38 <= 0.5f ? f26 + (1.0f - f29) * f30 - f33 * f26 * (1.0f - 2.0f * f38) * (1.0f - f35) : f26 + (1.0f - f29) * f30 + (2.0f * f30 - f33) * (f29 * f42 - f26)));
                float f48 = f29 == 0.0f ? f31 : (f33 == 0.0f ? f27 : (f39 <= 0.5f ? f27 + (1.0f - f29) * f31 - f33 * f27 * (1.0f - 2.0f * f39) * (1.0f - f36) : f27 + (1.0f - f29) * f31 + (2.0f * f31 - f33) * (f29 * f43 - f27)));
                float f49 = f29 == 0.0f ? f32 : (f33 == 0.0f ? f28 : (f40 <= 0.5f ? f28 + (1.0f - f29) * f32 - f33 * f28 * (1.0f - 2.0f * f40) * (1.0f - f37) : f28 + (1.0f - f29) * f32 + (2.0f * f32 - f33) * (f29 * f44 - f28)));
                float f50 = f47;
                float f51 = f48;
                float f52 = f49;
                float f53 = f34;
                ...
                arrn3[n14 + j] = (int)(f54 * 255.0f) << 16 | (int)(f55 * 255.0f) << 8 | (int)(f56 * 255.0f) << 0 | (int)(f57 * 255.0f) << 24;
                f9 += f2;
                f10 += f4;
            }
            f6 += f3;
