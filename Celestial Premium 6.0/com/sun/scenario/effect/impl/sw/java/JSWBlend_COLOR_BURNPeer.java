    ...
    public ImageData filter(Effect effect, RenderState renderState, BaseTransform baseTransform, Rectangle rectangle, ImageData ... arrimageData) {
        ...
        for (int i = 0; i < 0 + n12; ++i) {
            ...
            for (int j = 0; j < 0 + n11; ++j) {
                ...
                float f39 = f30 * f34;
                float f40 = f34 * f34;
                f13 = f30 == f27 ? f39 : (f31 == 0.0f ? 0.0f : ((f13 = f40 * (f30 - f27) / f31) >= f39 ? 0.0f : f39 - f13));
                f12 = f30 == f28 ? f39 : (f32 == 0.0f ? 0.0f : ((f12 = f40 * (f30 - f28) / f32) >= f39 ? 0.0f : f39 - f12));
                f11 = f30 == f29 ? f39 : (f33 == 0.0f ? 0.0f : ((f11 = f40 * (f30 - f29) / f33) >= f39 ? 0.0f : f39 - f11));
                float f41 = f36 += f13;
                float f42 = f37 += f12;
                float f43 = f38 += f11;
                float f44 = f35;
