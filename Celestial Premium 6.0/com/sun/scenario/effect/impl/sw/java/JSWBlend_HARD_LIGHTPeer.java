    ...
    public ImageData filter(Effect effect, RenderState renderState, BaseTransform baseTransform, Rectangle rectangle, ImageData ... arrimageData) {
        ...
        for (int i = 0; i < 0 + n12; ++i) {
            ...
            for (int j = 0; j < 0 + n11; ++j) {
                ...
                float f33 = 0.5f * f31;
                float f34 = f28 > f33 ? f28 + f27 * (f28 - f31) - f24 * (2.0f * f28 - f31 - 1.0f) : 2.0f * f24 * f28 + f24 * (1.0f - f31) + f28 * (1.0f - f27);
                float f35 = f29 > f33 ? f29 + f27 * (f29 - f31) - f25 * (2.0f * f29 - f31 - 1.0f) : 2.0f * f25 * f29 + f25 * (1.0f - f31) + f29 * (1.0f - f27);
                float f36 = f30 > f33 ? f30 + f27 * (f30 - f31) - f26 * (2.0f * f30 - f31 - 1.0f) : 2.0f * f26 * f30 + f26 * (1.0f - f31) + f30 * (1.0f - f27);
                float f37 = f34;
                float f38 = f35;
                float f39 = f36;
                float f40 = f32;
                ...
                arrn3[n14 + j] = (int)(f41 * 255.0f) << 16 | (int)(f42 * 255.0f) << 8 | (int)(f43 * 255.0f) << 0 | (int)(f44 * 255.0f) << 24;
                f9 += f2;
                f10 += f4;
            }
            f6 += f3;
