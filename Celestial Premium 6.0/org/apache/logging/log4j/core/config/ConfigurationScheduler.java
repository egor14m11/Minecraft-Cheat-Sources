    ...
    implements Runnable {
        ...
        public void run() {
            try {
                ...
                if (millis > 0L) {
                    LOGGER.debug("{} Cron thread woke up {} millis early. Sleeping", ConfigurationScheduler.this.name, millis);
                    try {
                        Thread.sleep(millis);
                    }
                ...
                LOGGER.error("{} caught error running command", ConfigurationScheduler.this.name, ex);
            }
            finally {
                ...
                LOGGER.debug("{} Cron expression {} scheduled to fire again at {}", ConfigurationScheduler.this.name, this.cronExpression.getCronExpression(), fireDate);
