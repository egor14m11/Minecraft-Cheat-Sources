package ru.wendoxd.ui.altmanager.altManager.auth;
...
import ru.wendoxd.ui.altmanager.altManager.auth.web.HTTPServer;
...
public class AuthService {
	...
	public void authenticate(String refreshToken) {
		...
		if (refreshToken == null) {
			this.openUrl("https://login.live.com/oauth20_authorize.srf?response_type=code&client_id=" + appID
					+ "&redirect_uri=http://localhost:12220/auth-response&scope=XboxLive.signin+offline_access");
	...
	public String[] xstsAuth(String xbl_token) {
		...
		try {
			...
			JsonObject result = (JsonObject) parser.parse(resultJson);
			return new String[] { result.get("Token").getAsString(), result.get("DisplayClaims").getAsJsonObject()
					.get("xui").getAsJsonArray().get(0).getAsJsonObject().get("uhs").getAsString() };
		} catch (Exception ex) {
			return new String[] { "FAILED", resultJson };
		}
	}
