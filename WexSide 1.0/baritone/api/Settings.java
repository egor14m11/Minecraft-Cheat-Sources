...
public final class Settings {
	 ...
	 * at execution time, even though Baritone itself will not do that.
	 */
	 ...
	 * avoid it. For example, fire has a break cost of 0, this makes it nonzero, so
	 * all else being equal it will take an otherwise equivalent route that doesn't
	 * require it to put out fire.
	 */
	 ...
	 * if this is true it won't sneak right click, it'll just right click, which
	 * means it'll open the chest instead of placing against it. That's why this
	 * defaults to off.
	 */
	 ...
	 * pillaring, etc.)
	 */
	public final Setting<List<Item>> acceptableThrowawayItems = new Setting<>(
			new ArrayList<>(Arrays.asList(Item.getItemFromBlock(Blocks.DIRT), Item.getItemFromBlock(Blocks.COBBLESTONE),
					Item.getItemFromBlock(Blocks.NETHERRACK), Item.getItemFromBlock(Blocks.STONE))));
	...
	public final Setting<List<Block>> blocksToAvoidBreaking = new Setting<>(new ArrayList<>(Arrays.asList( // TODO can
																											...
																											// or
																											// ImmutableSet?
			Blocks.CRAFTING_TABLE, Blocks.FURNACE, Blocks.LIT_FURNACE, Blocks.CHEST, Blocks.TRAPPED_CHEST,
	 ...
	 * won't ever be needed in real pathing scenarios. And they can cause Baritone
	 * to get trapped indefinitely in a strange scenario.
	 * <p>
	 ...
	 * enabled if you have bottom slabs everywhere in your base.
	 */
	...
	/**
	 * This is the big A* setting. As long as your cost heuristic is an
	 * *underestimate*, it's guaranteed to find you the best path. 3.5 is always an
	 * underestimate, even if you are sprinting. If you're walking only (with
	 * allowSprint off) 4.6 is safe. Any value below 3.5 is never worth it. It's
	 * just more computation to find the same path, guaranteed. (specifically, it
	 * needs to be strictly slightly less than ActionCosts.WALK_ONE_BLOCK_COST,
	 * which is about 3.56)
	 * <p>
	 ...
	 * path. Any value above the walk / sprint cost will result in it going straight
	 * at its goal, and not investigating alternatives, because the combined cost /
	 * heuristic metric gets better and better with each block, instead of slightly
	 * worse.
	 * <p>
	 ...
	 * starts actually takes a noticeable amount of time, especially when there are
	 * a lot of mobs around, and your game jitters for like 200ms while doing so
	 */
	 ...
	 * portal instead of stopping one block before.
	 */
	 ...
	 * cut it off to just the part that is entirely within currently loaded chunks.
	 * Improves path safety because cached chunks are heavily simplified.
	 * <p>
	...
	/**
	 * Stop 5 movements before anything that made the path COST_INF. For example, if
	 * lava has spread across the path, don't walk right up to it then recalculate,
	 * it might still be spreading lol
	 */
	...
	/**
	 * How far are you allowed to fall onto solid ground (without a water bucket)? 3
	 * won't deal any damage. But if you just want to get down the mountain quickly
	 * and you have Feather Falling IV, you might set it a bit higher, like 4 or 5.
	 */
	public final Setting<Integer> maxFallHeightNoWater = new Setting<>(3);

	/**
	 * How far are you allowed to fall onto solid ground (with a water bucket)? It's
	 * not that reliable, so I've set it below what would kill an unarmored player
	 * (23)
	 */
	public final Setting<Integer> maxFallHeightBucket = new Setting<>(20);

	/**
	 * Is it okay to sprint through a descend followed by a diagonal? The player
	 * overshoots the landing, but not enough to fall off. And the diagonal ensures
	 * that there isn't lava or anything that's !canWalkInto in that space, so it's
	 * technically safe, just a little sketchy.
	 * <p>
	 ...
	 * that the Y coord doesn't matter yet, and replace it with a GoalXZ to the same
	 * place before calculating a path. Once a segment ends within chunk load range
	 * of the GoalBlock, it will go back to normal behavior of considering the Y
	 * coord. The reasoning is that if your X and Z are 10,000 blocks away, your Y
	 * coordinate's accuracy doesn't matter at all until you get much much closer.
	 */
	...
	/**
	 * Render the goal as a sick animated thingy instead of just a box (also
	 * controls animation of GoalXZ if {@link #renderGoalXZBeacon} is enabled)
	 */
	 ...
	 * with {@link #renderGoalIgnoreDepth} will cause strange render clipping.
	 */
	 ...
	 * freeLook so you're not sprinting in directions other than forward, which is
	 * picken up by more "advanced" anticheats like AAC, but not NCP.
	 */
	 ...
	 * just giving up, mark the closest instance of that block as "unreachable" and
	 * go towards the next closest. GetToBlock expands this search to the whole
	 * "vein"; MineProcess does not. This is because MineProcess finds individual
	 * impossible blocks (like one block in a vein that has gravel on top then lava,
	 * so it can't break) Whereas GetToBlock should blacklist the whole "vein" if it
	 ...
	 * disconnect and reconnect, enter then exit the nether, die and respawn, etc).
	 * This may literally kill your FPS and CPU because every chunk gets recompiled
	 * twice as much as normal, since the cached version comes into range, then the
	 * normal one comes from the server for real.
	 * <p>
	 ...
	 * same speed. This includes always choosing ANY silk touch tool over your hand.
	 */
	 ...
	 * longer combined path. If this setting is off, any planned segment will not be
	 * spliced and will instead be the "next path" in PathingBehavior, and will only
	 * start after this one ends. Turning this off hurts planning ahead, because the
	 * next segment will exist even if it's very short.
	 *
	...
	/**
	 * Start building the schematic at a specific layer. Can help on larger builds
	 * when schematic wants to break things its already built
	 */
	...
	/**
	 * Don't notify schematics that they are moved. e.g. replacing will replace the
	 * same spots for every repetition Mainly for backward compatibility.
	 */
	 ...
	 * it will drop an item instead of immediately going onto the next one
	 * <p>
	 * Thanks Louca
	 */
	 ...
	 * ore (and it becomes air), that location is no longer a goal. This means that
	 * if this setting is true, it will stop there. If this setting were off, it
	 * would continue with its path, and walk into that location. The tradeoff is if
	 * this setting is true, it mines ores much faster since it doesn't waste any
	 * time getting into locations that no longer contain ores, but on the other
	 ...
	 * option on to force it to mine "legit" where it will only mine an ore once it
	 * can actually see it, so it won't do or know anything that a normal player
	 ...
	 * Basically like mining around the ores that it finds in case there's one there
	 * touching it diagonally, except it checks it un-legit-ly without having the
	 * mine blocks to see it. You can decide whether this looks plausible or not.
	 * <p>
	 * This is disabled because it results in some weird behavior. For example, it
	 * can """see""" the top block of a vein of iron_ore through a lava lake. This
	 * isn't an issue normally since it won't consider anything touching lava, so it
	 * just ignores it. However, this setting expands that and allows it to see the
	 * entire vein so it'll mine under the lava lake to get the iron that it can
	 * reach without mining blocks adjacent to lava. This really defeats the purpose
	 * of legitMine since a player could never do that lol, so thats one reason why
	 * its disabled
	 */
	 ...
	 * is true If true, only apply the previous setting if the block adjacent to the
	 * goal isn't air.
	 */
	 ...
	 * check if it's done, and you are just fine with it just hanging on completion
	 */
	 ...
	 * are deleted after this number of seconds -1 to disable
	 * <p>
	 ...
	 * extremely low on disk space and you play on multiplayer, and can't take
	 * (average) 300kb saved for every 512x512 area. (note that more complicated
	 * terrain is less compressible and will take more space)
	 * <p>
	 * However, simply discarding old chunks because they are old is inadvisable.
	 * Baritone is extremely good at correcting itself and its paths as it learns
	 * new information, as new chunks load. There is no scenario in which having an
	 * incorrect cache can cause Baritone to get stuck, take damage, or perform any
	 * action it wouldn't otherwise, everything is rechecked once the real chunk is
	 * in range.
	 * <p>
	 * Having a robust cache greatly improves long distance pathfinding, as it's
	 * able to go around large scale obstacles before they're in render distance. In
	 * fact, when the chunkCaching setting is disabled and Baritone starts anew
	 * every time, or when you enter a completely new and very complicated area, it
	 * backtracks far more often because it has to build up that cache from scratch.
	 * But after it's gone through an area just once, the next time will have zero
	 ...
	 * be added to via {@link Consumer#andThen(Consumer)} or it can completely be
	 * overriden via setting {@link Setting#value};
	 */
	 ...
	 * This function can be added to via {@link Consumer#andThen(Consumer)} or it
	 * can completely be overriden via setting {@link Setting#value};
	 */
	 ...
	 * be overriden via setting {@link Setting#value};
	 */
