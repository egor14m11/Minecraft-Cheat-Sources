...
public interface MovementHelper extends ActionCosts, Helper {
	...
	static boolean avoidBreaking(BlockStateInterface bsi, int x, int y, int z, IBlockState state) {
		...
		return Baritone.settings().blocksToDisallowBreaking.value.contains(b) || b == Blocks.ICE // ice becomes water,
																									// and water can
																									// mess up the path
	...
	static boolean canWalkThrough(BlockStateInterface bsi, int x, int y, int z, IBlockState state) {
		...
		if (block == Blocks.FIRE || block == Blocks.TRIPWIRE || block == Blocks.WEB || block == Blocks.END_PORTAL
				|| block == Blocks.COCOA || block instanceof BlockSkull || block instanceof BlockTrapDoor
	...
	static boolean fullyPassable(CalculationContext context, int x, int y, int z) {
		return fullyPassable(context.bsi.access, context.bsi.isPassableBlockPos.setPos(x, y, z),
	...
	static boolean fullyPassable(IBlockAccess access, BlockPos pos, IBlockState state) {
		...
		// through
		if (block == Blocks.FIRE || block == Blocks.TRIPWIRE || block == Blocks.WEB || block == Blocks.VINE
				|| block == Blocks.LADDER || block == Blocks.COCOA || block instanceof BlockDoor
				|| block instanceof BlockFenceGate || block instanceof BlockSnow || block instanceof BlockLiquid
	...
	static boolean avoidWalkingInto(Block block) {
		return block instanceof BlockLiquid || block == Blocks.MAGMA || block == Blocks.CACTUS || block == Blocks.FIRE
				|| block == Blocks.END_PORTAL || block == Blocks.WEB;
	}
	...
	static void moveTowards(IPlayerContext ctx, MovementState state, BlockPos pos) {
		state.setTarget(new MovementTarget(new Rotation(RotationUtils
				...
				.getYaw(), ctx.player().rotationPitch), false)).setInput(Input.MOVE_FORWARD, true);
	}
	...
	static PlaceResult attemptToPlaceABlock(MovementState state, IBaritone baritone, BlockPos placeAt,
		...
		for (int i = 0; i < 5; i++) {
			...
			if (MovementHelper.canPlaceAgainst(ctx, against1)) {
				...
				RayTraceResult res = RayTraceUtils.rayTraceTowards(ctx.player(), place,
