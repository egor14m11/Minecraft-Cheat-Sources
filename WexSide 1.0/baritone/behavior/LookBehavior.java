...
public final class LookBehavior extends Behavior implements ILookBehavior {
	...
	public void onPlayerUpdate(PlayerUpdateEvent event) {
		...
		case PRE: {
			if (this.force) {
				...
				ctx.player().rotationYaw += ((Math.random() - 0.5) * 1) * sensitivity;
				ctx.player().rotationPitch += ((Math.random() - 0.5) * 1) * sensitivity;
				if (desiredPitch == oldPitch && !Baritone.settings().freeLook.value) {
					nudgeToLevel();
				}
				ctx.player().rotationPitch = MathHelper.clamp(ctx.player().rotationPitch, -90, 90);
				this.target = null;
			}
			if (silent) {
	...
	public void onPlayerRotationMove(RotationMoveEvent event) {
		if (this.target != null) {
			...
			// If we have antiCheatCompatibility on, we're going to use the target value
			// later in onPlayerUpdate()
			// Also the type has to be MOTION_UPDATE because that is called after JUMP
			if (!Baritone.settings().antiCheatCompatibility.value
					&& event.getType() == RotationMoveEvent.Type.MOTION_UPDATE && !this.force) {
				this.target = null;
			}
		}
	}
