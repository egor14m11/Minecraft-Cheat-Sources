...
public final class BuilderProcess extends BaritoneProcessHelper implements IBuilderProcess {
    ...
    private Optional<Placement> possibleToPlace(IBlockState toPlace, int x, int y, int z, BlockStateInterface bsi) {
        for (EnumFacing against : EnumFacing.values()) {
            ...
            if (!ctx.world().func_190527_a(toPlace.getBlock(), new BetterBlockPos(x, y, z), false, against, null)) {
                continue;
            }
    ...
    private Goal placementGoal(BlockPos pos, BuilderCalculationContext bcc) {
        ...
        for (EnumFacing facing : Movement.HORIZONTALS_BUT_ALSO_DOWN_____SO_EVERY_DIRECTION_EXCEPT_UP) {
            ...
            if (MovementHelper.canPlaceAgainst(ctx, pos.offset(facing)) && ctx.world().func_190527_a(bcc.getSchematic(pos.getX(), pos.getY(), pos.getZ(), current).getBlock(), pos, false, facing, null)) {
