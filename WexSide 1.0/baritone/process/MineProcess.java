...
import ru.wendoxd.modules.impl.player.BaritoneHandler;
...
public final class MineProcess extends BaritoneProcessHelper implements IMineProcess {
	...
	public BlockOptionalMetaLookup filter;
	public List<BlockPos> knownOreLocations;
	public List<BlockPos> blacklist; // inaccessible
	public Map<BlockPos, Long> anticipatedDrops;
	public BlockPos branchPoint;
	public GoalRunAway branchPointRunaway;
	...
	public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
		if (desiredQuantity > 0 && !BaritoneHandler.baritoneIgnoreInvetory.isEnabled(false)) {
			int curr = ctx.player().inventory.mainInventory.stream().filter(stack -> filter.has(stack))
		...
		if (calcFailed) {
			if (!knownOreLocations.isEmpty() && Baritone.settings().blacklistClosestOnFailure.value) {
				...
				if (Baritone.settings().notificationOnMineFail.value) {
					logNotification("Unable to find any path to " + filter
							+ ", blacklisting presumably unreachable closest instance...", true);
				}
		...
		if ((mineGoalUpdateInterval != 0 && tickCount++ % mineGoalUpdateInterval == 0) || rescan) { // big brain
	...
	public PathingCommand updateGoal() {
		...
		if (branchPoint == null) {
			 ...
			 * cool, path is over and we are at desired y branchPoint = playerFeet();
			 * branchPointRunaway = null; } else { return new GoalYLevel(y); }
			 */
	...
	public void rescan(List<BlockPos> already, CalculationContext context) {
		if (filter == null) {
			return;
		}
		if (Baritone.settings().legitMine.value) {
	...
	public boolean internalMiningGoal(BlockPos pos, CalculationContext context, List<BlockPos> locs) {
	...
	public Goal coalesce(BlockPos loc, List<BlockPos> locs, CalculationContext context) {
	...
	public static List<BlockPos> searchWorld(CalculationContext ctx, BlockOptionalMetaLookup filter, int max,
		...
		for (BlockOptionalMeta bom : filter.blocks()) {
			...
			if (CachedChunk.BLOCKS_TO_KEEP_TRACK_OF.contains(block)) {
				...
				// further than that
				locs.addAll(ctx.worldData.getCachedWorld().getLocationsOf(BlockUtils.blockToString(block),
						Baritone.settings().maxCachedWorldScanCount.value, pf.x, pf.z, 2));
			} else {
				untracked.add(block);
			}
		}
		...
		if (!untracked.isEmpty() || (Baritone.settings().extendCacheOnThreshold.value && locs.size() < max)) {
			locs.addAll(
					WorldScanner.INSTANCE.scanChunkRadius(ctx.getBaritone().getPlayerContext(), filter, max, 10, 32)); // maxSearchRadius
			// is
			// NOT
			// sq
		}
				...
				for (int z = playerFeet.getZ() - searchDist; z <= playerFeet.getZ() + searchDist; z++) {
					...
					if (filter.has(bsi.get0(x, y, z))) {
						...
						if ((Baritone.settings().legitMineIncludeDiagonals.value && knownOreLocations.stream()
								...
								|| RotationUtils.reachable(ctx.player(), pos, fakedBlockReachDistance).isPresent()) {
	...
	public static List<BlockPos> prune(CalculationContext ctx, List<BlockPos> locs2, BlockOptionalMetaLookup filter,
		...
		for (int dx = -radius; dx <= radius; dx++) {
			for (int dy = -radius; dy <= radius; dy++) {
				for (int dz = -radius; dz <= radius; dz++) {
					if (Math.abs(dx) + Math.abs(dy) + Math.abs(dz) <= radius && MovementHelper
