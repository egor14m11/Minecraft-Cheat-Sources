			...
			if (nbttagcompound1.hasKey("Items", 9)) {
				...
				for (ItemStack itemstack : nonnulllist) {
					if (!itemstack.isEmpty()) {
						++j;
						if (!HUD.selected.get(4)) {
							if (i <= 4) {
								++i;
								p_190948_3_.add(
										String.format("%s x%d", itemstack.getDisplayName(), itemstack.getStackSize()));
							}
						} else {
							p_190948_3_.add(
									String.format("%s x%d", itemstack.getDisplayName2(), itemstack.getStackSize()));
						}
					}
				}

				if (j - i > 0 && !HUD.selected.get(4)) {
