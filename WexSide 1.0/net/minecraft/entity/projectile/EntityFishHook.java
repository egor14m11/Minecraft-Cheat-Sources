...
import ru.wendoxd.modules.impl.visuals.ESP;
import ru.wendoxd.modules.impl.visuals.ESP.PredictedPosition;
...
public class EntityFishHook extends Entity {
	...
	public List<PredictedPosition> predictedPositions = new ArrayList();
	...
	public void buildPositions(int ticks) {
		int i = 0;
		double prevLastPosX = this.lastTickPosX;
		double prevLastPosY = this.lastTickPosY;
		double prevLastPosZ = this.lastTickPosZ;
		double prevPrevPosX = this.prevPosX;
		double prevPrevPosY = this.prevPosY;
		double prevPrevPosZ = this.prevPosZ;
		...
		int prevTicksInGround = this.ticksInGround;
		int prevTicksInAir = this.ticksInAir;
		int prevTicksExisted = this.ticksExisted;
		float prevYaw = this.rotationYaw;
		float prevPitch = this.rotationPitch;
		float prevPrevYaw = this.prevRotationYaw;
		float prevPrevPitch = this.prevRotationPitch;
		this.predictTick = true;
		int old = ticksCatchableDelay;
		boolean ch = isCollidedHorizontally;
		State prevState = field_190627_av;
		this.predictedPositions.clear();
		float w = fishApproachAngle;
		while (i < ticks) {
			this.onUpdate();
			PredictedPosition pos = new PredictedPosition();
			pos.pos = this.getPositionVector();
			pos.tick = i;
			pos.color = new ColorShell();
			pos.color.setRGBA(ESP.predict.getColor().getRed(), ESP.predict.getColor().getGreen(),
					ESP.predict.getColor().getBlue(), ESP.predict.getColor().getAlpha());
			if (ESP.predict.getColor().isRainbow())
				pos.color.updateRainbow(i * 80L);
			this.predictedPositions.add(pos);
			if (i == 0) {
				this.breaked = false;
			}
			if (breaked) {
		...
		this.fishApproachAngle = w;
		this.ticksCatchableDelay = old;
		this.caughtEntity = null;
		this.isCollidedHorizontally = ch;
		this.field_190627_av = prevState;
		this.prevRotationYaw = prevPrevYaw;
		this.prevRotationPitch = prevPrevPitch;
		this.rotationYaw = prevRotationYaw;
		this.rotationPitch = prevRotationPitch;
		this.predictTick = false;
		this.lastTickPosX = prevLastPosX;
		this.lastTickPosY = prevLastPosY;
		this.lastTickPosZ = prevLastPosZ;
		this.prevPosX = prevPrevPosX;
		this.prevPosY = prevPrevPosY;
		this.prevPosZ = prevPrevPosZ;
		this.posX = prevPosX;
		this.posY = prevPosY;
		this.posZ = prevPosZ;
		...
		this.inGround = prevInGround;
		this.ticksInGround = prevTicksInGround;
		this.ticksInAir = prevTicksInAir;
		this.ticksExisted = prevTicksExisted;
	}
	...
	public void onUpdate() {
		if (ESP.predicts.get(2) && !predict && world.isRemote) {
			predict = true;
			buildPositions(150);
		}
		super.onUpdate();
