...
import ru.wendoxd.modules.impl.visuals.JumpCircles;
...
public abstract class EntityLivingBase extends Entity {
	...
	public void swingArm(EnumHand hand) {
		float end = this.getArmSwingAnimationEnd();
		if (this instanceof EntityPlayerSP) {
			end = Math.max(Minecraft.getMinecraft().player.getCooldownPeriod(), 10);
		}
		if (!this.isSwingInProgress || this.swingProgressInt >= end / 4f || this.swingProgressInt < 0) {
	...
	protected void updateArmSwingProgress() {
		...
		if (this instanceof EntityPlayerSP) {
			i = Math.max(Minecraft.getMinecraft().player.getCooldownPeriod(), 10);
		}
	...
	protected void jump() {
		...
		if (this == Minecraft.getMinecraft().player) {
			...
			if (JumpCircles.visuals_entitiestab.get(5).jumpcircles.isEnabled()) {
				JumpCircles.circles.add(new JumpCircles.Circle(getInterpolatePositionVector()));
			}
	...
	public boolean overrideMotionY;
				...
				if (!ItemStack.areItemStacksEqual(itemstack1, itemstack)) {
					...
					case HAND:
						this.handInventory.set(entityequipmentslot.getIndex(),
								itemstack1.isEmpty() ? ItemStack.field_190927_a : itemstack1.copy());
						break;
	...
	public float getSwingProgress2(float partialTickTime) {
		float f = this.swingProgress - this.prevSwingProgress;
