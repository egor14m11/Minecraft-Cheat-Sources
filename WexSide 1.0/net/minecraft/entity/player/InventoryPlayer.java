...
public class InventoryPlayer implements IInventory {
	...
	private boolean canMergeStacks(ItemStack stack1, ItemStack stack2) {
		return !stack1.isEmpty() && this.stackEqualExact(stack1, stack2) && stack1.isStackable()
				&& stack1.getStackSize() < stack1.getMaxStackSize()
				&& stack1.getStackSize() < this.getInventoryStackLimit();
	}
	...
	public int clearMatchingItems(@Nullable Item itemIn, int metadataIn, int removeCount,
		...
		for (int j = 0; j < this.getSizeInventory(); ++j) {
			...
			if (!itemstack.isEmpty() && (itemIn == null || itemstack.getItem() == itemIn)
				...
				int k = removeCount <= 0 ? itemstack.getStackSize()
						: Math.min(removeCount - i, itemstack.getStackSize());
				i += k;
		...
		if (!this.itemStack.isEmpty()) {
			...
			int l = removeCount <= 0 ? this.itemStack.getStackSize()
					: Math.min(removeCount - i, this.itemStack.getStackSize());
			i += l;
			...
			try {
				if (p_191971_2_.isItemDamaged()) {
					...
					while (true) {
						...
						if (p_191971_2_.isEmpty() || p_191971_2_.getStackSize() >= i) {
							break;
						}
					}
