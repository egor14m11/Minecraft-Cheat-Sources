...
import ru.wendoxd.modules.impl.visuals.JumpCircles.Circle;
...
public abstract class EntityPlayer extends EntityLivingBase {
	...
	public EntityItem dropItem(boolean dropAll) {
		return this.dropItem(this.inventory.decrStackSize(this.inventory.currentItem,
				dropAll && !this.inventory.getCurrentItem().isEmpty() ? this.inventory.getCurrentItem().getStackSize()
						: 1),
				false, true);
	}
		...
		if (this.isSpectator()) {
			...
			if (p_190775_1_.processInitialInteract(this, p_190775_2_)) {
				if (this.capabilities.isCreativeMode && itemstack == this.getHeldItem(p_190775_2_)
						&& itemstack.getStackSize() < itemstack1.getStackSize()) {
					itemstack.func_190920_e(itemstack1.getStackSize());
				}
		...
		if (targetEntity.canBeAttackedWithItem()) {
			if (!targetEntity.hitByEntity(this)) {
				...
				if (f > 0.0F || f1 > 0.0F) {
					...
					boolean flag2 = flag && !ground && fd != 0 && !this.isOnLadder() && !this.isInWater()
					...
					if (flag5) {
						if (i > 0) {
							...
							EventPlayerMotion event = new EventPlayerMotion(0.6, 0.6, false);
							EventManager.call(event);

							this.motionX *= event.getMotionX();
							this.motionZ *= event.getMotionZ();
							this.setSprinting(event.isSprint());
						}
						if (flag3) {
