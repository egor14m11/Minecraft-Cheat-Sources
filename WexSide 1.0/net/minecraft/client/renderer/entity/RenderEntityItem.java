...
import ru.wendoxd.modules.impl.visuals.ItemPhysics;
...
public class RenderEntityItem extends Render<EntityItem>
{
    ...
    private int transformModelCount(EntityItem itemIn, double p_177077_2_, double p_177077_4_, double p_177077_6_, float p_177077_8_, IBakedModel p_177077_9_)
    {
        ...
        if (item == null) {
            ...
            float f1 = ItemPhysics.itemphysics.isEnabled(false) ? 0 : MathHelper.sin(((float) itemIn.getAge() + p_177077_8_) / 10F + itemIn.hoverStart) * 0.1F + 0.1F;
            float f2 = p_177077_9_.getItemCameraTransforms().getTransform(ItemCameraTransforms.TransformType.GROUND).scale.y;
            GlStateManager.translate((float) p_177077_2_, (float) p_177077_4_ + f1 + 0.25F * f2, (float) p_177077_6_);

            if (!(ItemPhysics.itemphysics.isEnabled(false)) && (flag || this.renderManager.options != null)) {
                float f3 = (((float) itemIn.getAge() + p_177077_8_) / 20F + itemIn.hoverStart) * (180F / (float) Math.PI);
                GlStateManager.rotate(f3, 0, 1, 0);
            }

            this.shadowSize = ItemPhysics.itemphysics.isEnabled(false) ? 0 : 0.15F;
    ...
    {
        if (!ItemPhysics.itemphysics.isEnabled(false)) {
            ...
            RenderEntityItem.rotation = (System.nanoTime() - RenderEntityItem.tick) / 3000000.0 * 0.5F;
