...
import ru.wendoxd.events.impl.render.*;
import ru.wendoxd.modules.impl.movement.Speed;
...
public class EntityRenderer implements IResourceManagerReloadListener, Runnable {
	...
	public void updateRenderer() {
		...
		this.prevThirdPerson = this.thirdPerson;
		boolean changeSign = false;
		if (mc.gameSettings.thirdPersonView != 0) {
			if (mc.gameSettings.thirdPersonView != this.lastThirdPerson) {
				this.thirdPerson = 0;
				this.lastThirdPerson = mc.gameSettings.thirdPersonView;
			}
		} else {
			this.thirdPerson = 0;
		}
		int sign = changeSign ? -1 : 1;
		this.thirdPerson = MathHelper.clamp(this.thirdPerson + sign, 0, 10);
	...
	public void getMouseOver(float partialTicks) {
		...
		if (entity != null && this.mc.world != null) {
			...
			EventEntityDistanceAttack event = new EventEntityDistanceAttack(3);
			EventManager.call(event);



			if (this.pointedEntity != null && flag && vec3d.distanceTo(vec3d3) > event.getDistance()) {
				this.pointedEntity = null;
	...
	private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
		if (this.debugView) {
			...
			if (useFOVSetting) {
				...
				if (Config.isDynamicFov() && !Speed.strafes()) {
			...
			if (flag) {
				...
				zoomDelta = (float) MathHelper.clampedLerp(zoomDelta, 6, 2 * (1. / Minecraft.getDebugFPS()));
			} else {
				zoomDelta = (float) MathHelper.clampedLerp(zoomDelta, 1, 3 * (1. / Minecraft.getDebugFPS()));
				if (Config.zoomMode) {
	...
	private void hurtCameraEffect(float partialTicks) {
		...
		EventGameOverlay event = new EventGameOverlay(EventGameOverlay.OverlayType.Hurt);
		EventManager.call(event);
	...
	private void orientCamera(float partialTicks) {
		...
		if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
			...
			double d3 = WexSide.createAnimation(
					(this.prevThirdPerson + (this.thirdPerson - this.prevThirdPerson) * partialTicks) / 10.);
			d3 = 4 * d3;
			if (this.mc.gameSettings.debugCamEnable) {
				...
				for (int i = 0; i < 8; ++i) {
					...
					EventGameOverlay event = new EventGameOverlay(EventGameOverlay.OverlayType.CameraBounds);
					EventManager.call(event);



					if (raytraceresult != null && !event.isCanceled()) {
		...
		if (this.lightmapUpdateNeeded) {
			...
			if (world != null) {
				...
				for (int i = 0; i < 256; ++i) {
					...
					float f3 = world.provider.getLightBrightnessTable()[i % 16]
		...
		if (!this.mc.skipRenderWorld) {
			{
				...
				if (this.mc.world == null) {
					...
					if (!this.mc.gameSettings.hideGUI || this.mc.currentScreen != null) {
						...
						this.setupOverlayRendering(2);
						res = new ScaledResolution(mc, 2);
						EventManager.call(new EventRender2D(res));
						if (blurEnabled) {
							FramebufferShell.blur(this);
							blurEnabled = false;
						}
	...
	private void setupFog(int startCoords, float partialTicks) {
		EventGameOverlay event = new EventGameOverlay(EventGameOverlay.OverlayType.Fog);
		EventManager.call(event);
		...
		if (f >= 0.0F) {
			...
			EventGameOverlay event2 = new EventGameOverlay(EventGameOverlay.OverlayType.WaterFog);
			...
			EventGameOverlay event3 = new EventGameOverlay(EventGameOverlay.OverlayType.LavaFog);
	...
	private void checkLoadVisibleChunks(Entity p_checkLoadVisibleChunks_1_, float p_checkLoadVisibleChunks_2_,
		...
		if (Keyboard.isKeyDown(61) && Keyboard.isKeyDown(38)) {
			...
			Minecraft_actionKeyF3.setValue(this.mc, Boolean.TRUE);
		}
	}
	...
	public static void drawNameplateNameTags(FontRenderer fontRendererIn, String str, float x, float y, float z,
			int verticalShift, float viewerYaw, float viewerPitch, boolean isThirdPersonFrontal, boolean isSneaking,
			Entity entity) {
		GlStateManager.pushMatrix();
		GlStateManager.translate(x, y, z);
	...
	public void func_190565_a(ItemStack p_190565_1_) {
		EventGameOverlay event = new EventGameOverlay(EventGameOverlay.OverlayType.TotemPop);
		EventManager.call(event);
	...
	@Override
	public void run() {
		EventManager.call(new EventBlur());
	}
}
