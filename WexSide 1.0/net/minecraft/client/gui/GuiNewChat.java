...
public class GuiNewChat extends Gui {
				...
				for (int i1 = 0; i1 + this.scrollPos < this.drawnChatLines.size() && i1 < i; ++i1) {
					...
					if (chatline != null) {
						...
						if (j1 < 200 || flag) {
							...
							if (l1 > 3) {
								int j2 = -i1 * 9;
								EventGameOverlay event = new EventGameOverlay(EventGameOverlay.OverlayType.ChatRect);
								...
								chatline.setPosY((float) interpolate(chatline.getPosY(), j2 - 8.5F, 0.1F));
								chatline.setPosX((float) interpolate(chatline.getPosX(), 0, 0.1F));
								this.mc.fontRendererObj.drawStringWithShadow(s, flag ? 0 : chatline.getPosX(),
										flag ? j2 - 9.5F + f1 : chatline.getPosY(), 0xFFFFFF + (l1 << 24));
	...
	public double interpolate(double startValue, double endValue, double fraction) {
		return startValue + (endValue - startValue) * WexSide.createAnimation(fraction);
	}

	public void updateChatLines() {
		int i = this.getLineCount();
		for (int i1 = 0; i1 + this.scrollPos < this.drawnChatLines.size() && i1 < i; ++i1) {
			ChatLine chatline = this.drawnChatLines.get(i1 + this.scrollPos);
			chatline.updateAnimation();
		}
	}
	...
	public void printChatMessageWithOptionalDeletion(ITextComponent chatComponent, int chatLineId) {
		if ((chatComponent.getUnformattedText().contains("Это действие запрещено на спавне.")
				|| chatComponent.getUnformattedText().contains("Sorry")) && Spider.spider.isEnabled(false)) {
			return;
		}
