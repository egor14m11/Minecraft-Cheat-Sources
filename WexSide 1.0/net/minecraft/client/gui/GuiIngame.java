...
public class GuiIngame extends Gui {
	...
	public void renderGameOverlay(float partialTicks) {
		...
		if ((GuiPlayerTabOverlay.y_animation.getDrop() > 0)
		...
		if (gamesettings.thirdPersonView == 0) {
			...
			if (gamesettings.showDebugInfo && !gamesettings.hideGUI && !this.mc.player.hasReducedDebug()
				...
				GlStateManager.tryBlendFuncSeparate(GlStateManager.SourceFactor.ONE_MINUS_DST_COLOR,
						...
						GlStateManager.DestFactor.ZERO);
				GlStateManager.enableAlpha();
				if (!Crosshair.crosshair.isEnabled(false))
	...
	private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
		if (NoOverlay.nooverlay.isEnabled(false) && NoOverlay.type.get(14))
			return;
		scaledRes = new ScaledResolution(mc, 2);
		...
		for (Score score1 : collection) {
			...
			if (mc.getCurrentServerData() != null && StreamerMode.type.get(4)
					&& StreamerMode.streamermode.isEnabled(false)) {
				...
				if (s.contains(mc.player.getName())) {
					s1 = s.replace(mc.player.getName(), "1");
				}
			}
	...
	private void renderPumpkinOverlay(ScaledResolution scaledRes) {
		EventGameOverlay event = new EventGameOverlay(EventGameOverlay.OverlayType.PumpkinOverlay);
