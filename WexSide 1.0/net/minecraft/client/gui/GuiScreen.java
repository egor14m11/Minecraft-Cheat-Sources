...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback {
	...
	public void handleMouseInput() throws IOException {
		...
		if (Mouse.getEventButtonState()) {
			...
			EventManager.call(new EventClickMouse(MenuAPI.mouseX, MenuAPI.mouseY, this.eventButton));
		} else if (k != -1) {
			if (this.mc.gameSettings.touchscreen && --this.touchValue > 0) {
				return;
			}
	...
	public void drawWorldBackground(int tint) {
		try {
			if (this.mc.world != null) {
				if (!NoOverlay.type.get(15)) {
					RenderUtils.drawRect(0, 0, width, height, RenderUtils.rgba(15 * animation.get(),
							15 * animation.get(), 15 * animation.get(), 205 * animation.get()));
				}
			} else {
