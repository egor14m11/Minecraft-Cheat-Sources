...
import ru.wendoxd.modules.impl.player.ItemScroller;
...
public abstract class GuiContainer extends GuiScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		RenderUtils.sizeAnimation(width, height, animation.getDrop());
		int i = this.guiLeft;
		int j = this.guiTop;
		...
		for (int i1 = 0; i1 < this.inventorySlots.inventorySlots.size(); ++i1) {
			...
			if (this.isMouseOverSlot(slot, mouseX, mouseY) && slot.canBeHovered()) {
				if (ItemScroller.itemscroller.isEnabled(false)) {
					if (Mouse.isButtonDown(0) && Keyboard.isKeyDown(mc.gameSettings.keyBindSneak.getKeyCode())
							&& mc.currentScreen != null) {
						if (timer.hasReached(ItemScroller.delay.getFloatValue())) {
							GuiContainer container = (GuiContainer) mc.currentScreen;
							container.handleMouseClick(slot, slot.slotNumber, 0, ClickType.QUICK_MOVE);
							timer.reset();
						}
					}
				}

				if (canShiftDropItems(this)) {
					if (ItemScroller.itemscroller.isEnabled(false)) {
						if (Keyboard.isKeyDown(mc.gameSettings.keyBindSprint.getKeyCode())
								&& mc.currentScreen != null) {
							if (timer.hasReached(ItemScroller.delay.getFloatValue())) {
								GuiContainer container = (GuiContainer) mc.currentScreen;
								container.handleMouseClick(slot, -999, 0, ClickType.PICKUP);
								timer.reset();
							}
						}
					}
	...
	private boolean canShiftDropItems(GuiContainer gui) {
		if (!GuiScreen.isShiftKeyDown() || Mouse.getEventButton() != 0) {
			return false;
		}
		...
		boolean isOutsideGui = mouseAbsX < left || mouseAbsY < top || mouseAbsX >= left + xSize
				|| mouseAbsY >= top + ySize;
		if (isOutsideGui) {
			this.getSlotAtPosition(mouseAbsX - left, mouseAbsY - top);
		}
		return false;
	}
