...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
		...
		for (IBaritone ibaritone : BaritoneAPI.getProvider().getAllBaritones()) {
			...
			if (player != null && player.connection == (NetHandlerPlayClient) (Object) this) {
				ibaritone.getGameEventHandler().onChunkEvent(
						new ChunkEvent(EventState.POST, ChunkEvent.Type.POPULATE_FULL, pos.chunkXPos, pos.chunkZPos));
			}
		}
	}
		...
		for (IBaritone ibaritone : BaritoneAPI.getProvider().getAllBaritones()) {
			...
			if (player != null && player.connection == (NetHandlerPlayClient) (Object) this) {
				ibaritone.getGameEventHandler()
						.onChunkEvent(new ChunkEvent(EventState.PRE,
								packetIn.doChunkLoad() ? ChunkEvent.Type.POPULATE_FULL
	...
	public void handleBlockChange(SPacketBlockChange packetIn) {
		PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
		try {
			if (this.clientWorldController.getBlockState(packetIn.getBlockPosition()).getBlock() == Blocks.AIR
					&& packetIn.getBlockState().getBlock() != Blocks.AIR
					&& BaritoneHandler.baritoneAntiRg.isEnabled(false) && BaritoneHandler.baritone.isEnabled(false)) {
				IBaritone baritone = BaritoneAPI.getProvider().getBaritoneForPlayer(Minecraft.getMinecraft().player);
				if (baritone.getMineProcess().isActive() && baritone.getMineProcess() instanceof MineProcess) {
		...
		if (packetIn.getWindowId() == -1) {
			...
			if (packetIn.getWindowId() == 0 && packetIn.getSlot() >= 36 && i < 45) {
				if (!itemstack.isEmpty()) {
					...
					if (itemstack1.isEmpty() || itemstack1.getStackSize() < itemstack.getStackSize()) {
	...
	public void handlePlayerListItem(SPacketPlayerListItem packetIn) {
		...
		for (SPacketPlayerListItem.AddPlayerData spacketplayerlistitem$addplayerdata : packetIn.getEntries()) {
			try {
				EventManager.call(new EventPacketPlayerList(spacketplayerlistitem$addplayerdata, packetIn.getAction()));
			} catch (Exception e) {
				ChatUtils.addChatMessage("Error while handling spacketplayerlistitem");
			}
