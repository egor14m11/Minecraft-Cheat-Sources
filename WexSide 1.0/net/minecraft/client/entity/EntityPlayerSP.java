...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public boolean overrideSprintState;
	public Runnable run;
	 ...
	 */
	public int sprintingTicksLeft, ticksInAir;
	...
	private boolean skipServerUpdate, allowedForcePlayerUpdate;
	...
	public boolean lastUpdatedOnGround;
	...
	public void forceUpdatePlayerServerPosition(double posX, double posY, double posZ, float yaw, float pitch,
			boolean onGround) {
	...
	private void onUpdateWalkingPlayer() {
		EventEntitySync event = new EventEntitySync(rotationYaw, rotationPitch, posX, posY, posZ, onGround);
		EventManager.call(event);
		...
		if (this.isCurrentViewEntity()) {
			...
			if (this.isRiding()) {
				...
				this.lastUpdatedFallDistance = this.fallDistance;
				this.connection.sendPacket(new CPacketPlayer.PositionRotation(event.getPosX(), event.getPosY(),
						event.getPosZ(), event.getYaw(), event.getPitch(), event.isOnGround()));
				var = 2;
			} else if (flag2) {
				this.lastUpdatedFallDistance = this.fallDistance;
				this.connection.sendPacket(new CPacketPlayer.Position(event.getPosX(), event.getPosY(), event.getPosZ(),
						event.isOnGround()));
				var = 3;
			} else if (flag3) {
				this.connection
						.sendPacket(new CPacketPlayer.Rotation(event.getYaw(), event.getPitch(), event.isOnGround()));
				var = 4;
			...
			if (var != 0) {
				...
				ru.wendoxd.modules.impl.movement.Timer.m(var);
			}
			if (flag2) {
	...
	public void setAuraCallback(Runnable run) {
		this.run = run;
	}
	...
	public void swingArmClientSided(EnumHand hand) {
		super.swingArm(hand);
	}
	...
	public void onLivingUpdate() {
		...
		if (this.isHandActive() && !this.isRiding()) {
			EventMovementInput event = new EventMovementInput(0.2F, 0.2F);
			EventManager.call(event);



			this.movementInput.moveStrafe *= event.getMoveStrafe();
