...
import Celestial.cmd.impl.FakeNameCommand;
import Celestial.event.EventManager;
import Celestial.event.events.impl.player.EventUpdateLiving;
import Celestial.module.impl.Combat.KeepSprint;
import Celestial.module.impl.Util.NameProtect;
import Celestial.module.impl.Movement.Speed;
import Celestial.module.impl.Movement.Strafe;
import Celestial.module.impl.Movement.FreeCam;
import Celestial.module.impl.Movement.NoClip;
import Celestial.module.impl.Player.NoPush;
import Celestial.wavecapes.CapeHolder;
import Celestial.wavecapes.stim.StickSimulation;
...
public abstract class EntityPlayer extends EntityLivingBase implements CapeHolder {
    ...
    public void onUpdate() {
        this.noClip = Smertnix.instance.featureManager.getFeature(NoClip.class).isEnabled() || Smertnix.instance.featureManager.getFeature(FreeCam.class).isEnabled() || this.isSpectator();
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (flag5) {
                        if (i > 0) {
                            ...
                            if (Smertnix.instance.featureManager.getFeature(KeepSprint.class).isEnabled()) {
    ...
    public String getName() {
        ...
        if (this == (Minecraft.getMinecraft()).player && FakeNameCommand.canChange) {
            ...
            str = (Smertnix.instance.featureManager.getFeature(NameProtect.class).isEnabled() && NameProtect.myName.getCurrentValue() && this == (Minecraft.getMinecraft()).player) ? "Protected" : this.gameProfile.getName();
        }
        return str;
    }
    ...
    public boolean isPushedByWater() {
        if (Smertnix.instance.featureManager.getFeature(NoPush.class).isEnabled() && NoPush.water.getCurrentValue()) {
            return false;
        }
        return !this.capabilities.isFlying;
    }
    ...
    public float getEyeHeight() {
        ...
        if (this.isPlayerSleeping()) {
            ...
            if (this.isElytraFlying() || this.height == 0.6F) {
                if (Smertnix.instance.featureManager.getFeature(Strafe.class).isEnabled() || Smertnix.instance.featureManager.getFeature(Speed.class).isEnabled() && Speed.speedMode.currentMode.equals("Matrix Elytra")) {
                    f = 1.62F;
                } else {
                    f = 0.4F;
                }
