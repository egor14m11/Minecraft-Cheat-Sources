    ...
    public MovementState updateState(MovementState state) {
        ...
        if (this.ctx.playerFeet().equals(this.dest)) {
            ...
            if (this.ctx.playerFeet().equals(this.src.offset(this.direction)) || this.ctx.player().posY - (double)this.src.y > 1.0E-4) {
                if (!MovementHelper.canWalkOn(this.ctx, this.dest.down()) && !this.ctx.player().onGround && MovementHelper.attemptToPlaceABlock(state, this.baritone, this.dest.down(), true, false) == PlaceResult.READY_TO_PLACE) {
