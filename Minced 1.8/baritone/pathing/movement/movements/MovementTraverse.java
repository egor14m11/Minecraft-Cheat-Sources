...
extends Movement {
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (pb0.getBlock() instanceof BlockFenceGate || pb1.getBlock() instanceof BlockFenceGate) {
            ...
            if (betterBlockPos != null && (rotation = RotationUtils.reachable(this.ctx, betterBlockPos)).isPresent()) {
        ...
        if (isTheBridgeBlockThere) {
            ...
            if (feet.getY() != this.dest.getY() && ladder && (destDown.getBlock() == Blocks.VINE || destDown.getBlock() == Blocks.LADDER)) {
                BlockPos blockPos = against = destDown.getBlock() == Blocks.VINE ? (BetterBlockPos) MovementPillar.getAgainst(new CalculationContext(this.baritone), this.dest.down()) : this.dest.offset(destDown.getValue(BlockLadder.FACING).getOpposite());
                if (against == null) {
        ...
        PlaceResult p = MovementHelper.attemptToPlaceABlock(state, this.baritone, this.dest.down(), false, true);
        if ((p == PlaceResult.READY_TO_PLACE || dist1 < 0.6) && !((Boolean)Baritone.settings().assumeSafeWalk.value).booleanValue()) {
