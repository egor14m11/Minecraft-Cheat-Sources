...
implements IBuilderProcess {
    ...
    public void buildOpenSchematic() {
        if (SchematicaHelper.isSchematicaPresent()) {
            ...
            if (schematic.isPresent()) {
                ...
                this.build(schematic.get().getFirst().toString(), Baritone.settings().mapArtMode.value != false ? new MapArtSchematic(s) : s, schematic.get().getSecond());
            } else {
    ...
    private Optional<Tuple<BetterBlockPos, Rotation>> toBreakNearPlayer(BuilderCalculationContext bcc) {
        ...
        for (int dx = -5; dx <= 5; ++dx) {
            int dy;
            int n = dy = Baritone.settings().breakFromAbove.value != false ? -1 : 0;
            while (dy <= 5) {
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel, int recursions) {
        ...
        if (!this.recalc(bcc = new BuilderCalculationContext())) {
            if (Baritone.settings().buildInLayers.value.booleanValue() && this.layer < this.realSchematic.heightY()) {
            ...
            if (repeat.equals(new Vec3i(0, 0, 0)) || max != -1 && this.numRepeats >= max) {
                ...
                if (Baritone.settings().desktopNotifications.value.booleanValue() && Baritone.settings().notificationOnBuildFinished.value.booleanValue()) {
        ...
        if ((goal = this.assemble(bcc, this.approxPlaceable.subList(0, 9))) == null && (goal = this.assemble(bcc, this.approxPlaceable, true)) == null) {
            if (Baritone.settings().skipFailedLayers.value.booleanValue() && Baritone.settings().buildInLayers.value.booleanValue() && this.layer < this.realSchematic.heightY()) {
            ...
            for (int z = 0; z < this.schematic.lengthZ(); ++z) {
                for (int x = 0; x < this.schematic.widthX(); ++x) {
                    ...
                    if (bcc.bsi.worldContainsLoadedChunk(blockX, blockZ)) {
                        ...
                        if (this.incorrectPositions.size() <= Baritone.settings().incorrectSize.value) continue;
                        return;
                    }
                    if (this.observedCompleted.contains(BetterBlockPos.longHash(blockX, blockY, blockZ)) || Baritone.settings().buildSkipBlocks.value.contains(this.schematic.desiredState(x, y, z, current, this.approxPlaceable).getBlock())) continue;
                    this.incorrectPositions.add(new BetterBlockPos(blockX, blockY, blockZ));
                    if (this.incorrectPositions.size() <= Baritone.settings().incorrectSize.value) continue;
                    return;
                }
    ...
    private Goal breakGoal(BlockPos pos, BuilderCalculationContext bcc) {
        if (Baritone.settings().goalBreakFromAbove.value.booleanValue() && bcc.bsi.get0(pos.up()).getBlock() instanceof BlockAir && bcc.bsi.get0(pos.up(2)).getBlock() instanceof BlockAir) {
