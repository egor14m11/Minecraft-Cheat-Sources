...
public class EntityPlayerSP extends AbstractClientPlayer
{
	...
	private void onUpdateWalkingPlayer() {
		MotionEvent motionEvent = new MotionEvent(EventType.PRE, posX, posY, posZ, rotationYaw, rotationPitch,
				onGround);
		...
		double x = motionEvent.getPosX();
		double y = motionEvent.getPosY();
		double z = motionEvent.getPosZ();
		...
		if (this.isCurrentViewEntity()) {
			...
			double d2 = y - this.lastReportedPosZ;
			...
			if (this.isRiding()) {
				...
				this.connection.sendPacket(new CPacketPlayer.PositionRotation(x, y, z,
						yaw, pitch, onGround));
			} else if (flag2) {
				this.connection.sendPacket(
						new CPacketPlayer.Position(x, y, z, onGround));
			} else if (flag3) {
				this.connection
		...
		EventManager.call(new MotionEvent(EventType.POST, x, y, z, yaw, pitch,
				onGround));
	}   
    ...
    {
		PushOutBlockEvent pushOutEvent = new PushOutBlockEvent();
		EventManager.call(pushOutEvent);
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.isHandActive() && !this.isRiding() && !Wrapper.getModule().get("NoSlowdown").isEnabled())
        {
    ...
    public boolean isInWater() {
    	PlayerInWaterEvent event = new PlayerInWaterEvent(super.isInWater());
