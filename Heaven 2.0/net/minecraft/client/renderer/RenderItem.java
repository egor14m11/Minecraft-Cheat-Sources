...
public class RenderItem implements IResourceManagerReloadListener
{
    ...
    protected void renderItemModel(ItemStack stack, IBakedModel bakedmodel, ItemCameraTransforms.TransformType transform, boolean leftHanded)
    {
        if (!stack.isEmpty())
        {
            ...
            try {
                if (Wrapper.MC.getRenderManager().options.thirdPersonView == 0) {
                    ViewModel viewModel = ((ViewModel)Wrapper.getModule().get("ViewModel"));
                    if (viewModel.isEnabled())
                        if (viewModel.offhandValid(stack))
                            GlStateManager.scale(viewModel.oscale.getValue(), viewModel.oscale.getValue(), viewModel.oscale.getValue());
                        else
                            GlStateManager.scale(viewModel.scale.getValue(), viewModel.scale.getValue(), viewModel.scale.getValue());
                }
