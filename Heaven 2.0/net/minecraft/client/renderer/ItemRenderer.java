...
import org.spray.heaven.features.module.modules.render.HitAnimation;
import org.spray.heaven.features.module.modules.render.NoRender;
import org.spray.heaven.features.module.modules.render.ViewModel;
...
public class ItemRenderer
{
    ...
    private void transformFirstPerson(EnumHandSide p_187453_1_, float p_187453_2_)
    {
		...
		HitAnimation hitVisualize = Wrapper.getModule().get("HitAnimation");
		KillAura killaura = Wrapper.getModule().get("KillAura");
		boolean hitAnimation = hitVisualize.isEnabled() &&
				killaura.isEnabled() && killaura.getTarget() != null;
		if (hitAnimation) {
			GlStateManager.rotate((float) i * f1 * 10.0F, 0.0F, 0.0F, 1.0F);
	...
	private void transformSideFirstPerson(EnumHandSide p_187459_1_, float p_187459_2_, ItemStack stack) {
		int i = p_187459_1_ == EnumHandSide.RIGHT ? 1 : -1;
		HitAnimation hitVisualize = Wrapper.getModule().get("HitAnimation");
		KillAura killaura = Wrapper.getModule().get("KillAura");
		boolean hitAnimation = hitVisualize.isEnabled() &&
				killaura.isEnabled() && killaura.getTarget() != null && mc.player.getHeldItemMainhand() == stack;
		float swingMultiply = hitAnimation ? 0f : -0.6f;
		ViewModel viewModel = ((ViewModel) Wrapper.getModule().get("ViewModel"));
		if (viewModel.isEnabled()) {
			if (viewModel.offhandValid(stack))
				GlStateManager.translate((float) i * 0.56F + viewModel.oxRotation.getValue(),
						-0.52F + p_187459_2_ * swingMultiply + viewModel.oyRotation.getValue(),
						-0.72F + viewModel.ozRotation.getValue());
			else
				GlStateManager.translate((float) i * 0.56F + viewModel.xRotation.getValue(),
						-0.52F + p_187459_2_ * swingMultiply + viewModel.yRotation.getValue(),
						-0.72F + viewModel.zRotation.getValue());
		} else
			GlStateManager.translate((float) i * 0.56F, -0.52F + p_187459_2_ * swingMultiply, -0.72F);
	}
    /**
    ...
    public void renderItemInFirstPerson(AbstractClientPlayer p_187457_1_, float p_187457_2_, float p_187457_3_, EnumHand p_187457_4_, float p_187457_5_, ItemStack stack, float p_187457_7_)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(p_187457_4_))
        {
            ...
            else
            {
				...
				boolean hitAnimation = hitVisualize.isEnabled() &&
						killaura.isEnabled() && killaura.getTarget() != null && 
						mc.player.getHeldItemMainhand() == stack;
                ...
                if (p_187457_1_.isHandActive() && p_187457_1_.getItemInUseCount() > 0 && p_187457_1_.getActiveHand() == p_187457_4_)
                {
                    ...
                    {
                        case NONE:
                            this.transformSideFirstPerson(enumhandside, p_187457_7_, stack);
                            break;
                ...
                else
                {
    				...
    				if (hitAnimation) {
    					int side = p_187457_1_.getPrimaryHand() == EnumHandSide.RIGHT ? 1 : -1;
    					GlStateManager.translate(side * -0.14142136f, 0.18f, 0.14142136f);
    					GlStateManager.rotate(-102.25f, 1.0F, 0.0F, 0.0F);
    					GlStateManager.rotate(side * -8.365f, 0.0F, 1.0F, 0.0F);
    					GlStateManager.rotate(side * 88.05f, 0.0F, 0.0F, 1.0F);
    				}
                }

                this.renderItemSide(p_187457_1_, stack, flag1 ? ItemCameraTransforms.TransformType.FIRST_PERSON_RIGHT_HAND : ItemCameraTransforms.TransformType.FIRST_PERSON_LEFT_HAND, !flag1);
            }
    ...
    {
		NoRender noRender = Wrapper.getModule().get("NoRender");
		if (noRender.isEnabled() && noRender.noFire.isToggle())
			return;
