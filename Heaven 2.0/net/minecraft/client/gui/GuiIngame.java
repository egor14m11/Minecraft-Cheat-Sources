...
import org.spray.heaven.events.ChatMessageEvent;
...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        if (!Wrapper.getModule().get("PotionEffects").isEnabled())
        ...
        boolean noscore = noRender.isEnabled() && noRender.noScoreboard.isToggle();
        if (scoreobjective1 != null && !noscore)
        {
        ...
        EventManager.call(new HudRenderEvent(i, j, partialTicks));
    }
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes)
    {
        ...
        for (Score score1 : collection)
        {
            ...
            if (nameProtect.isEnabled() && nameProtect.scoreboard.isToggle()) {
               this.getFontRenderer().drawString(s1.replace(Wrapper.getPlayer().getName(), nameProtect.PROTECT), l1, k, 553648127);
            } else
    ...
    {
    	ChatMessageEvent chatEvent = new ChatMessageEvent(p_191742_2_.getUnformattedText());
    	EventManager.register(chatEvent);
