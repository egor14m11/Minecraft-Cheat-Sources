...
import org.spray.heaven.events.BlockBBEvent;
import org.spray.heaven.features.module.modules.player.AutoTool;
import org.spray.heaven.features.module.modules.player.NoInteract;
...
public class Block
{
    ...
    protected static void addCollisionBoxToList(BlockPos pos, AxisAlignedBB entityBox, List<AxisAlignedBB> collidingBoxes, @Nullable AxisAlignedBB blockBox)
    {
        if (blockBox != NULL_AABB)
        {
        	...
        	BlockBBEvent blockBBEvent = new BlockBBEvent(pos, axisalignedbb);
    ...
    {
    	NoInteract noInteract = Wrapper.getModule().get("NoInteract");
        for (Block block : Objects.requireNonNull(noInteract.getRightClickableBlocks())) {
            if (state.getBlock() != block || !noInteract.isEnabled()) continue;
            return false;
        }
    ...
    public float getPlayerRelativeBlockHardness(IBlockState state, EntityPlayer player, World worldIn, BlockPos pos)
    {
        ...
        {
        	AutoTool autoTool = Wrapper.getModule().get("AutoTool");
            return !player.canHarvestBlock(state) ? player.getDigSpeed(state, (autoTool.isEnabled() && autoTool.silentSwitch.isToggle()) ? player.inventory.getStackInSlot(autoTool.itemIndex) : player.getHeldItemMainhand()) / f / 100.0F : player.getDigSpeed(state,(autoTool.isEnabled() && autoTool.silentSwitch.isToggle()) ? player.inventory.getStackInSlot(autoTool.itemIndex) : player.getHeldItemMainhand()) / f / 30.0F;
        }
    }
