...
import org.spray.heaven.events.MoveEvent;
...
public abstract class Entity implements ICommandSender
{
    ...
    {
    	MoveEvent moveEvent = new MoveEvent(EventType.PRE, moverType, x, y, z);
        ...
        else
        {
            ...
            if ((moverType == MoverType.SELF || moverType == MoverType.PLAYER) && this.onGround && (this.isSneaking() || Wrapper.getModule().get("SafeWalk").isEnabled()) && this instanceof EntityPlayer)
            {
            ...
            StepEvent stepEvent = new StepEvent(EventType.PRE, stepHeight);
            if (this == Wrapper.getPlayer()) 
                EventManager.call(stepEvent);

            if (stepEvent.getStepHeight() > 0.0F && flag && (d2 != x || d4 != z))
            {
                double d14 = x;
                double d6 = y;
                double d7 = z;
                ...
                StepEvent postStepEvent = new StepEvent(EventType.POST, stepHeight);
                if (this == Wrapper.getPlayer()) 
                    EventManager.call(postStepEvent);
            }
        ...
        EventManager.call(new MoveEvent(EventType.POST, moverType, x, y, z));
    }
    ...
    protected final Vec3d getVectorForRotation(float pitch, float yaw)
    {
    	...
    	if (event.isCancelled() && this == Wrapper.getPlayer()) {
    		yaw = event.getYaw();
    ...
    {
        return ((HitBoxes)Wrapper.getModule().get("HitBoxes")).getCustomSize(this);
    }
