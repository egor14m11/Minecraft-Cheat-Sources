...
import volcano.summer.screen.mp3.TimerUtil;
import volcano.summer.screen.mp3.resources.Decoder;
import volcano.summer.screen.mp3.resources.Frame;
import volcano.summer.screen.mp3.resources.SampleBuffer;
import volcano.summer.screen.mp3.resources.SoundStream;
			...
			playingThread = new Thread() {
				...
				public void run() {
					...
					try {
						...
						//
						// source is null at this point only if first frame is
						// null
						// this means that probably the file is not a mp3

						if (playingSource == null) {
							LOGGER.log(Level.INFO,
									"source is null because first frame is null, so probably the file is not a mp3");
						}
					...
					if (skipForwardAllowed) {
						Summer.tellPlayer("Now playing " + MP3Player.this.getCurrentTitleAndArtist() + ".");
						skipForward();
					}
				}
			};
