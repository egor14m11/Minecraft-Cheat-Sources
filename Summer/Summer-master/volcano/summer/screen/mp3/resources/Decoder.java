...
public class Decoder implements DecoderErrors {
	...
	protected FrameDecoder retrieveDecoder(final Frame header, final SoundStream stream, final int layer)
		...
		case 2: {
			if (this.l2decoder == null) {
				(this.l2decoder = new LayerIIDecoder()).create(stream, header, this.filter1, this.filter2, this.output,
						0);
			}
		...
		case 1: {
			if (this.l1decoder == null) {
				(this.l1decoder = new LayerIDecoder()).create(stream, header, this.filter1, this.filter2, this.output,
						0);
			}
