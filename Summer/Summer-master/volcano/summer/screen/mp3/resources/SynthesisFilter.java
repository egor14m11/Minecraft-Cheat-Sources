...
final class SynthesisFilter {
	...
	private void compute_pcm_samples0(final Obuffer buffer) {
		...
		for (int i = 0; i < 32; ++i) {
			...
			final float pcm_sample = (vp[0 + dvp] * dp[0] + vp[15 + dvp] * dp[1] + vp[14 + dvp] * dp[2]
	...
	private void compute_pcm_samples1(final Obuffer buffer) {
		...
		for (int i = 0; i < 32; ++i) {
			final float[] dp = SynthesisFilter.d16[i];
			final float pcm_sample = (vp[1 + dvp] * dp[0] + vp[0 + dvp] * dp[1] + vp[15 + dvp] * dp[2]
	...
	private void compute_pcm_samples2(final Obuffer buffer) {
		...
		for (int i = 0; i < 32; ++i) {
			final float[] dp = SynthesisFilter.d16[i];
			final float pcm_sample = (vp[2 + dvp] * dp[0] + vp[1 + dvp] * dp[1] + vp[0 + dvp] * dp[2]
	...
	private void compute_pcm_samples3(final Obuffer buffer) {
		...
		for (int i = 0; i < 32; ++i) {
			final float[] dp = SynthesisFilter.d16[i];
			final float pcm_sample = (vp[3 + dvp] * dp[0] + vp[2 + dvp] * dp[1] + vp[1 + dvp] * dp[2]
	...
	private void compute_pcm_samples4(final Obuffer buffer) {
		...
		for (int i = 0; i < 32; ++i) {
			final float[] dp = SynthesisFilter.d16[i];
			final float pcm_sample = (vp[4 + dvp] * dp[0] + vp[3 + dvp] * dp[1] + vp[2 + dvp] * dp[2]
	...
	private void compute_pcm_samples5(final Obuffer buffer) {
		...
		for (int i = 0; i < 32; ++i) {
			final float[] dp = SynthesisFilter.d16[i];
			final float pcm_sample = (vp[5 + dvp] * dp[0] + vp[4 + dvp] * dp[1] + vp[3 + dvp] * dp[2]
	...
	private void compute_pcm_samples6(final Obuffer buffer) {
		...
		for (int i = 0; i < 32; ++i) {
			final float[] dp = SynthesisFilter.d16[i];
			final float pcm_sample = (vp[6 + dvp] * dp[0] + vp[5 + dvp] * dp[1] + vp[4 + dvp] * dp[2]
					+ vp[3 + dvp] * dp[3] + vp[2 + dvp] * dp[4] + vp[1 + dvp] * dp[5] + vp[0 + dvp] * dp[6]
					+ vp[15 + dvp] * dp[7] + vp[14 + dvp] * dp[8] + vp[13 + dvp] * dp[9] + vp[12 + dvp] * dp[10]
					+ vp[11 + dvp] * dp[11] + vp[10 + dvp] * dp[12] + vp[9 + dvp] * dp[13] + vp[8 + dvp] * dp[14]
	...
	private void compute_pcm_samples7(final Obuffer buffer) {
		...
		for (int i = 0; i < 32; ++i) {
			final float[] dp = SynthesisFilter.d16[i];
			final float pcm_sample = (vp[7 + dvp] * dp[0] + vp[6 + dvp] * dp[1] + vp[5 + dvp] * dp[2]
					+ vp[4 + dvp] * dp[3] + vp[3 + dvp] * dp[4] + vp[2 + dvp] * dp[5] + vp[1 + dvp] * dp[6]
					+ vp[0 + dvp] * dp[7] + vp[15 + dvp] * dp[8] + vp[14 + dvp] * dp[9] + vp[13 + dvp] * dp[10]
					+ vp[12 + dvp] * dp[11] + vp[11 + dvp] * dp[12] + vp[10 + dvp] * dp[13] + vp[9 + dvp] * dp[14]
	...
	private void compute_pcm_samples8(final Obuffer buffer) {
		...
		for (int i = 0; i < 32; ++i) {
			final float[] dp = SynthesisFilter.d16[i];
			final float pcm_sample = (vp[8 + dvp] * dp[0] + vp[7 + dvp] * dp[1] + vp[6 + dvp] * dp[2]
					+ vp[5 + dvp] * dp[3] + vp[4 + dvp] * dp[4] + vp[3 + dvp] * dp[5] + vp[2 + dvp] * dp[6]
					+ vp[1 + dvp] * dp[7] + vp[0 + dvp] * dp[8] + vp[15 + dvp] * dp[9] + vp[14 + dvp] * dp[10]
					+ vp[13 + dvp] * dp[11] + vp[12 + dvp] * dp[12] + vp[11 + dvp] * dp[13] + vp[10 + dvp] * dp[14]
	...
	private void compute_pcm_samples9(final Obuffer buffer) {
		...
		for (int i = 0; i < 32; ++i) {
			final float[] dp = SynthesisFilter.d16[i];
			final float pcm_sample = (vp[9 + dvp] * dp[0] + vp[8 + dvp] * dp[1] + vp[7 + dvp] * dp[2]
					+ vp[6 + dvp] * dp[3] + vp[5 + dvp] * dp[4] + vp[4 + dvp] * dp[5] + vp[3 + dvp] * dp[6]
					+ vp[2 + dvp] * dp[7] + vp[1 + dvp] * dp[8] + vp[0 + dvp] * dp[9] + vp[15 + dvp] * dp[10]
					+ vp[14 + dvp] * dp[11] + vp[13 + dvp] * dp[12] + vp[12 + dvp] * dp[13] + vp[11 + dvp] * dp[14]
	...
	private void compute_pcm_samples10(final Obuffer buffer) {
		...
		for (int i = 0; i < 32; ++i) {
			final float[] dp = SynthesisFilter.d16[i];
			final float pcm_sample = (vp[10 + dvp] * dp[0] + vp[9 + dvp] * dp[1] + vp[8 + dvp] * dp[2]
					+ vp[7 + dvp] * dp[3] + vp[6 + dvp] * dp[4] + vp[5 + dvp] * dp[5] + vp[4 + dvp] * dp[6]
					+ vp[3 + dvp] * dp[7] + vp[2 + dvp] * dp[8] + vp[1 + dvp] * dp[9] + vp[0 + dvp] * dp[10]
					+ vp[15 + dvp] * dp[11] + vp[14 + dvp] * dp[12] + vp[13 + dvp] * dp[13] + vp[12 + dvp] * dp[14]
	...
	private void compute_pcm_samples11(final Obuffer buffer) {
		...
		for (int i = 0; i < 32; ++i) {
			final float[] dp = SynthesisFilter.d16[i];
			final float pcm_sample = (vp[11 + dvp] * dp[0] + vp[10 + dvp] * dp[1] + vp[9 + dvp] * dp[2]
					+ vp[8 + dvp] * dp[3] + vp[7 + dvp] * dp[4] + vp[6 + dvp] * dp[5] + vp[5 + dvp] * dp[6]
					+ vp[4 + dvp] * dp[7] + vp[3 + dvp] * dp[8] + vp[2 + dvp] * dp[9] + vp[1 + dvp] * dp[10]
					+ vp[0 + dvp] * dp[11] + vp[15 + dvp] * dp[12] + vp[14 + dvp] * dp[13] + vp[13 + dvp] * dp[14]
	...
	private void compute_pcm_samples12(final Obuffer buffer) {
		...
		for (int i = 0; i < 32; ++i) {
			final float[] dp = SynthesisFilter.d16[i];
			final float pcm_sample = (vp[12 + dvp] * dp[0] + vp[11 + dvp] * dp[1] + vp[10 + dvp] * dp[2]
					+ vp[9 + dvp] * dp[3] + vp[8 + dvp] * dp[4] + vp[7 + dvp] * dp[5] + vp[6 + dvp] * dp[6]
					+ vp[5 + dvp] * dp[7] + vp[4 + dvp] * dp[8] + vp[3 + dvp] * dp[9] + vp[2 + dvp] * dp[10]
					+ vp[1 + dvp] * dp[11] + vp[0 + dvp] * dp[12] + vp[15 + dvp] * dp[13] + vp[14 + dvp] * dp[14]
	...
	private void compute_pcm_samples13(final Obuffer buffer) {
		...
		for (int i = 0; i < 32; ++i) {
			final float[] dp = SynthesisFilter.d16[i];
			final float pcm_sample = (vp[13 + dvp] * dp[0] + vp[12 + dvp] * dp[1] + vp[11 + dvp] * dp[2]
	...
	private void compute_pcm_samples14(final Obuffer buffer) {
		...
		for (int i = 0; i < 32; ++i) {
			final float[] dp = SynthesisFilter.d16[i];
			final float pcm_sample = (vp[14 + dvp] * dp[0] + vp[13 + dvp] * dp[1] + vp[12 + dvp] * dp[2]
	...
	private void compute_pcm_samples15(final Obuffer buffer) {
		...
		for (int i = 0; i < 32; ++i) {
			final float[] dp = SynthesisFilter.d16[i];
			final float pcm_sample = (vp[15 + dvp] * dp[0] + vp[14 + dvp] * dp[1] + vp[13 + dvp] * dp[2]
