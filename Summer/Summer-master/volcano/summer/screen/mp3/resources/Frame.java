...
public final class Frame {
	...
	void parseVBR(final byte[] firstframe) throws BitstreamException {
		...
		try {
			...
			if (xing.equals(new String(tmp))) {
				...
				if ((flags[3] & 0x1) != 0x0) {
					...
					this.h_vbr_frames = ((tmp[0] << 24 & 0xFF000000) | (tmp[1] << 16 & 0xFF0000)
							| (tmp[2] << 8 & 0xFF00) | (tmp[3] & 0xFF));
					length += 4;
				}
				if ((flags[3] & 0x2) != 0x0) {
					System.arraycopy(firstframe, offset + length, tmp, 0, tmp.length);
					this.h_vbr_bytes = ((tmp[0] << 24 & 0xFF000000) | (tmp[1] << 16 & 0xFF0000) | (tmp[2] << 8 & 0xFF00)
							| (tmp[3] & 0xFF));
					length += 4;
				}
				if ((flags[3] & 0x4) != 0x0) {
				...
				if ((flags[3] & 0x8) != 0x0) {
					System.arraycopy(firstframe, offset + length, tmp, 0, tmp.length);
					this.h_vbr_scale = ((tmp[0] << 24 & 0xFF000000) | (tmp[1] << 16 & 0xFF0000) | (tmp[2] << 8 & 0xFF00)
							| (tmp[3] & 0xFF));
					length += 4;
				}
			}
		...
		try {
			...
			if (vbri.equals(new String(tmp))) {
				...
				length2 += 4;
				System.arraycopy(firstframe, offset + length2, tmp, 0, tmp.length);
				this.h_vbr_frames = ((tmp[0] << 24 & 0xFF000000) | (tmp[1] << 16 & 0xFF0000) | (tmp[2] << 8 & 0xFF00)
						| (tmp[3] & 0xFF));
				length2 += 4;
			}
	...
	public int calculate_framesize() {
		if (this.h_layer == 1) {
			this.framesize = 12 * Frame.bitrates[this.h_version][0][this.h_bitrate_index]
					/ Frame.frequencies[this.h_version][this.h_sample_frequency];
			...
			this.framesize = 144 * Frame.bitrates[this.h_version][this.h_layer - 1][this.h_bitrate_index]
					/ Frame.frequencies[this.h_version][this.h_sample_frequency];
			...
			if (this.h_layer == 3) {
				if (this.h_version == 1) {
					this.nSlots = this.framesize - ((this.h_mode == 3) ? 17 : 32)
							- ((this.h_protection_bit != 0) ? 0 : 2) - 4;
				} else {
					this.nSlots = this.framesize - ((this.h_mode == 3) ? 9 : 17)
							- ((this.h_protection_bit != 0) ? 0 : 2) - 4;
				}
			} else {
				this.nSlots = 0;
	...
	public float ms_per_frame() {
		...
		final float[][] ms_per_frame_array = { { 8.707483f, 8.0f, 12.0f }, { 26.12245f, 24.0f, 36.0f },
