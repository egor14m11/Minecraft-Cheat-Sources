...
public final class SoundStream implements BitstreamErrors {
	...
	public SoundStream(InputStream in) {
		...
		this.header_pos = 0;
		this.bitmask = new int[] { 0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383, 32767, 65535,
				131071 };
	...
	public boolean isSyncCurrentPosition(final int syncmode) throws BitstreamException {
		...
		final int headerstring = (this.syncbuf[0] << 24 & 0xFF000000) | (this.syncbuf[1] << 16 & 0xFF0000)
				| (this.syncbuf[2] << 8 & 0xFF00) | (this.syncbuf[3] << 0 & 0xFF);
		try {
	...
	int syncHeader(final byte syncmode) throws BitstreamException {
		...
		int headerstring = (this.syncbuf[0] << 16 & 0xFF0000) | (this.syncbuf[1] << 8 & 0xFF00)
	...
	void parse_frame() throws BitstreamException {
		...
		for (int bytesize = this.framesize, k = 0; k < bytesize; k += 4) {
			...
			this.framebuffer[b++] = ((b2 << 24 & 0xFF000000) | (b3 << 16 & 0xFF0000) | (b4 << 8 & 0xFF00)
					| (b5 & 0xFF));
		}
		this.wordpointer = 0;
		this.bitindex = 0;
