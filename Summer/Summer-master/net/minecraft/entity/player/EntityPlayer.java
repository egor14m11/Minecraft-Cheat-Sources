...
public abstract class EntityPlayer extends EntityLivingBase {
				...
				if (var2 > 0.0F || var4 > 0.0F) {
					...
					if (var14) {
						if (var18 > 0) {
							...
							if (!Summer.moduleManager.getModule(NoSlowDown.class).state) {
								this.motionX *= 0.6D;
								this.motionZ *= 0.6D;
	...
	public boolean isEntityInsideOpaqueBlock() {
		return (!this.sleeping) && (super.isEntityInsideOpaqueBlock())
				&& (!Summer.moduleManager.getModule(Phase.class).getState())
	...
	public boolean isPushedByWater() {
		if (!this.capabilities.isFlying) {
			if (!Summer.moduleManager.getModule(Velocity.class).getState()) {
				return true;
			}
			if (Velocity.INSTANCE.water.getValue()) {
				return false;
			}
			return true;
		}
