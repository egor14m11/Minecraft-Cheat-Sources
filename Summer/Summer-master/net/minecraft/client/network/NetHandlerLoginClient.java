...
public class NetHandlerLoginClient implements INetHandlerLoginClient {
	...
	public void handleEncryptionRequest(S01PacketEncryptionRequest packetIn) {
		...
		if (MCLeaks.isAltActive()) {
			...
			String server = ((InetSocketAddress) this.field_147393_d.getRemoteAddress()).getHostName() + ":"
					+ ((InetSocketAddress) this.field_147393_d.getRemoteAddress()).getPort();
			try {
				String jsonBody = "{\"session\":\"" + mcLeaksSession + "\",\"mcname\":\"" + mcName
						+ "\",\"serverhash\":\"" + var5 + "\",\"server\":\"" + server + "\"}";
			...
			try {
				func_147391_c().joinServer(this.field_147394_b.getSession().getProfile(),
						this.field_147394_b.getSession().getToken(), var5);
			} catch (AuthenticationUnavailableException var7) {
				this.field_147393_d.closeChannel(new ChatComponentTranslation("disconnect.loginFailedInfo",
				...
				this.field_147393_d.closeChannel(
						new ChatComponentTranslation("disconnect.loginFailedInfo", new Object[] { var6.getMessage() }));
				return;
			}
		}
