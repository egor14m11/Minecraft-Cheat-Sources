...
public class ModAPI {
	...
	public static void redeem(final String token, final Callback<Object> callback) {
		...
		ModAPI.EXECUTOR_SERVICE.execute(new Runnable() {
			@Override
			public void run() {
				final URLConnection connection = preparePostRequest("http://auth.mcleaks.net/v1/redeem",
						"{\"token\":\"" + token + "\"}");
				if (connection == null) {
	...
	private static Object getResult(final URLConnection urlConnection) {
		try {
			...
			reader.close();
			final JsonElement jsonElement = (JsonElement) ModAPI.gson.fromJson(result.toString(),
